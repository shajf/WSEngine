...
static const char* process_cmd_line(int modid,void *mconfig,ngx_pool_t *p,ngx_pool_t *temp_pool,const char *l,cmd_parms *parms){
	...
	cmd = wsengine_find_command(modid,cmd_name);
...
static const char* process_config(int modid,void *mconfig,cmd_parms *parms,ngx_pool_t *p,ngx_pool_t *temp_pool){
    ...
    while ((rc = ngx_varbuf_cfg_getline(&vb, parms->config_file, max_len))
           == NGX_OK) {
        errval = process_cmd_line(modid,mconfig,p, temp_pool, vb.buf, parms);
...
const char* read_config(int modid,void *mconfig,ngx_pool_t *p,ngx_pool_t *ptemp,const char* filename){
    ...
    return process_config(modid,mconfig,&parms,p,ptemp);
}
