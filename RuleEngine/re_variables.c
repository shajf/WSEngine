 ...
 */
static int var_simple_generate_ex(msre_var *var, ngx_table_t *vartab, ngx_pool_t *mptmp,
    const char *value, int value_len)
{
    ...
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = value;
    rvar->value_len = value_len;
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
 ...
 */
static int var_simple_generate(msre_var *var, ngx_table_t *vartab, ngx_pool_t *mptmp,
    const char *value)
{
    if (value == NULL) return 0;
    return var_simple_generate_ex(var, vartab, mptmp, value, ngx_strlen(value));
}
...
static char *var_generic_list_validate(msre_ruleset *ruleset, msre_var *var) {
    ...
    if ((ngx_strlen(var->param) > 2)&&(var->param[0] == '/')
        &&(var->param[ngx_strlen(var->param) - 1] == '/'))
    { /* Regex. */
        ...
        pattern = ngx_pstrmemdup(ruleset->mp, var->param + 1, ngx_strlen(var->param + 1) - 1);
        ...
        if (regex == NULL) {
            size_t maxlen = ngx_strlen("Error compiling pattern (offset ): ")+NGX_INT64_LEN+ngx_strlen(errptr)+10;
            return ngx_psprintf(ruleset->mp,maxlen, "Error compiling pattern (offset u_%d): %s",
                erroffset, errptr);
        }
...
static int var_args_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    arr = ngx_table_elts(msr->arguments);
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            msre_var *rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));

            rvar->value = arg->value;
            rvar->value_len = arg->value_len;
            char *nname = log_escape_nq_ex(mptmp, arg->name, arg->name_len);
            size_t maxlen = ngx_strlen("ARGS:")+ngx_strlen(nname)+2;
            rvar->name = ngx_psprintf(mptmp, maxlen,"ARGS:%s", nname);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp,NGX_INT64_LEN+2, "%ud", combined_size);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            char *nname = log_escape_nq_ex(mptmp, arg->name, arg->name_len);
            size_t maxlen = ngx_strlen("ARGS_NAMES:")+ngx_strlen(nname)+2;
            rvar->name = ngx_psprintf(mptmp, maxlen,"ARGS_NAMES:%s",nname);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (ngx_strcmp("QUERY_STRING", arg->origin) != 0) continue;
        ...
        if (match) {
            ...
            rvar->value_len = arg->value_len;
            char *nname = log_escape_nq_ex(mptmp, arg->name, arg->name_len);
            size_t maxlen = ngx_strlen("ARGS_GET:")+ngx_strlen(nname)+2;

            rvar->name = ngx_psprintf(mptmp,maxlen,"ARGS_GET:%s", nname);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            rvar->value_len = arg->name_len;
            char *nname = log_escape_nq_ex(mptmp, arg->name, arg->name_len);
            size_t maxlen = ngx_strlen("ARGS_GET_NAMES:")+ngx_strlen(nname)+2;
            rvar->name = ngx_psprintf(mptmp, maxlen,"ARGS_GET_NAMES:%s", nname);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        /* Only BODY arguments */
        if (ngx_strcmp("BODY", arg->origin) != 0) continue;
        ...
        if (match) {
            ...
            rvar->value_len = arg->value_len;
            char *nname = log_escape_nq_ex(mptmp, arg->name, arg->name_len);
            size_t maxlen = ngx_strlen("ARGS_POST:")+ngx_strlen(nname)+2;
            rvar->name = ngx_psprintf(mptmp, maxlen,"ARGS_POST:%s", nname);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            rvar->value_len = arg->name_len;
            char *nname = log_escape_nq_ex(mptmp, arg->name, arg->name_len);
            size_t maxlen = ngx_strlen("ARGS_POST_NAMES:")+ngx_strlen(nname)+2;

            rvar->name = ngx_psprintf(mptmp, maxlen,"ARGS_POST_NAMES:%s", nname);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    if ((strcasecmp(var->param, "severity") == 0)&&(actionset->severity != -1)) {
        ...
        char *value = ngx_psprintf(mptmp, NGX_INT64_LEN+2,"%d", actionset->severity);
    ...
    if ((strcasecmp(var->param, "maturity") == 0)&&(actionset->maturity != -1)) {
        char *value = ngx_psprintf(mptmp, NGX_INT64_LEN+2,"%d", actionset->maturity);
        return var_simple_generate(var, vartab, mptmp, value);
    } else
    if ((strcasecmp(var->param, "accuracy") == 0)&&(actionset->accuracy != -1)) {
        char *value = ngx_psprintf(mptmp, NGX_INT64_LEN+2,"%d", actionset->accuracy);
        return var_simple_generate(var, vartab, mptmp, value);
    }
...
static int var_request_uri_raw_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    return var_simple_generate(var, vartab, mptmp,(const char*)ngx_pstrdup(mptmp,&msr->r->unparsed_uri));
}
...
{
    if (msr->txid != NULL) {
        return var_simple_generate(var, vartab, mptmp, msr->txid);
    }
...
static int var_request_uri_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp) /* dynamic */
{
    char *value = NULL;

    if (msr->r->args.len == 0) value = (char*)ngx_pstrdup(mptmp,&msr->r->uri);
    else value = ngx_psprintf(mptmp,msr->r->uri.len+msr->r->args.len+4,"%V?%V",&msr->r->uri,&msr->r->args);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    if (msr->msc_reqbody_processor == NULL) {
        rvar->value = ngx_pstrndup(mptmp, "");
        rvar->value_len = 0;
    } else {

        rvar->value = ngx_pstrndup(mptmp,msr->msc_reqbody_processor);
        rvar->value_len = ngx_strlen(rvar->value);
    }
...
/* SDBM_DELETE_ERROR */
static int var_sqlite_delete_error_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    msre_var *rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));

    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN+2,"%d", msr->msc_sqlite_delete_error);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    rvar->value = ngx_psprintf(mptmp,NGX_INT64_LEN+2,"%d", msr->msc_reqbody_error);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
...
static int var_reqbody_processor_error_msg_generate(modsec_rec *msr, msre_var *var,
    msre_rule *rule, ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    msre_var *rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    if (msr->msc_reqbody_error_msg == NULL) {
        ...
        rvar->value = ngx_pstrndup(mptmp,msr->msc_reqbody_error_msg); 
        rvar->value_len = ngx_strlen(rvar->value);
    }
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    if (var->param == NULL) {
        ...
        rvar->value = ngx_pstrndup(mptmp,"[XML document tree]");
        rvar->value_len = ngx_strlen(rvar->value);
        ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    for(i = 0; i < (ngx_uint_t)nodes->nodeNr; i++) {
        msre_var *rvar = NULL;
        char *content = NULL;
        ...
        if (content != NULL) {
            rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
            rvar->value =ngx_pstrndup(mptmp, content);
            xmlFree(content);
            rvar->value_len = ngx_strlen(rvar->value);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for(i = 0; i < msr->error_messages->nelts; i++) {
        ...
        if (fem != NULL) {
            rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
            rvar->value = ngx_pstrndup(mptmp, fem);
            rvar->value_len = ngx_strlen(rvar->value);
            ngx_table_addn(vartab, rvar->name, (void *)rvar);
...
static int var_remote_host_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
        ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    return var_simple_generate(var, vartab, mptmp, msr->remote_host);
}
...
static int var_remote_port_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%ud", msr->remote_port);
    return var_simple_generate(var, vartab, mptmp, value);
}
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    arr = ngx_table_elts(msr->tx_vars);
    te = (ngx_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            char *nname = log_escape_nq_ex(mptmp, str->name, str->name_len);

			rvar->name = ngx_psprintf(mptmp, ngx_strlen(nname)+ngx_strlen("TX:")+2,
			"TX:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    arr = ngx_table_elts(msr->geo_vars);
    te = (ngx_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        msc_string *str = (msc_string *)te[i].val;
        int match;
        ...
        if (match) {
            ...
            rvar->name = ngx_psprintf(mptmp,ngx_strlen("GEO:")+ngx_strlen(nname)+2, 
				"GEO:%s", nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    return var_simple_generate(var, vartab, mptmp,
                               ngx_psprintf(mptmp, NGX_INT64_LEN,"%d", msr->highest_severity));
}
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    target_col = (ngx_table_t *)ngx_table_get(msr->collections, "ip");
    if (target_col == NULL) return 0;

    arr = ngx_table_elts(target_col);
    te = (ngx_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        msc_string *str = (msc_string *)te[i].val;
        int match;
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp, ngx_strlen("IP:")+ngx_strlen(nname)+2,
				"IP:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    target_col = (ngx_table_t *)ngx_table_get(msr->collections, "session");
    if (target_col == NULL) return 0;
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp,ngx_strlen("SESSION:")+ngx_strlen(nname)+2, 
				"SESSION:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    target_col = (ngx_table_t *)ngx_table_get(msr->collections, "user");
    if (target_col == NULL) return 0;
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp,ngx_strlen("USER:")+ngx_strlen(nname)+2,
				"USER:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    target_col = (ngx_table_t *)ngx_table_get(msr->collections, "global");
    if (target_col == NULL) return 0;
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp, ngx_strlen("GLOBAL:")+ngx_strlen(nname)+2,
				"GLOBAL:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    target_col = (ngx_table_t *)ngx_table_get(msr->collections, "resource");
    if (target_col == NULL) return 0;
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp, ngx_strlen("RESOURCE:")+ngx_strlen(nname)+2, 
				"RESOURCE:%s",nname);
                ...
                (parts[i]->tmp_file_name != NULL))
        {
            ...
            else
            {
                if (var->param_data != NULL)
                {
                    ...
                    if (!(msc_regexec((msc_regex_t *)var->param_data,
                        parts[i]->name, ngx_strlen(parts[i]->name),
            ...
            if (match) {
                ...
                {   
                    ...
                    {
                        full_content = ngx_psprintf(mptmp,ngx_strlen(buf)+2,"%s", buf);
                    }
                    else
                    {
                        full_content = ngx_psprintf(mptmp, ngx_strlen(full_content)+ngx_strlen(buf)+2,"%s%s", full_content, buf);
                    }
                }
                fclose(file);
				...
				char *nname = log_escape_nq(mptmp, parts[i]->name);

                rvar->name = ngx_psprintf(mptmp, ngx_strlen("FILES_TMP_CONTENT:")+ngx_strlen(nname)+2,
					"FILES_TMP_CONTENT:%s",nname);
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if ((parts[i]->type == MULTIPART_FILE)&&(parts[i]->tmp_file_name != NULL)) {
            ...
            if (var->param == NULL) match = 1;
                ...
                if (var->param_data != NULL) { /* Regex. */
                    ...
                    if (!(msc_regexec((msc_regex_t *)var->param_data, parts[i]->name,
                        ngx_strlen(parts[i]->name), &my_error_msg) == PCRE_ERROR_NOMATCH)) match = 1;
                } else { /* Simple comparison. */
            ...
            if (match) {
				...
				rvar->name = ngx_psprintf(mptmp, ngx_strlen("FILES_TMPNAMES:")+ngx_strlen(nname)+2,
					"FILES_TMPNAMES:%s",nname);
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if (parts[i]->type == MULTIPART_FILE) {
            ...
            if (match) {
				...
				rvar->name = ngx_psprintf(mptmp, ngx_strlen("FILES:")+ngx_strlen(nname)+2,
					"FILES:%s",nname);
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if (parts[i]->type == MULTIPART_FILE) {
            ...
            if (match) {
                ...
                rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%u", parts[i]->tmp_file_size);
                rvar->value_len = ngx_strlen(rvar->value);
				char *nname = log_escape_nq(mptmp, parts[i]->name);

                rvar->name = ngx_psprintf(mptmp, ngx_strlen("FILES_SIZES:")+ngx_strlen(nname)+2,
					"FILES_SIZES:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if (parts[i]->type == MULTIPART_FILE) {
			...
			char *nname = log_escape_nq_ex(mptmp, parts[i]->name, rvar->value_len);

			rvar->name = ngx_psprintf(mptmp, ngx_strlen("FILES_NAMES:")+ngx_strlen(nname)+2,
				"FILES_NAMES:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%u", combined_size);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
...
static ngx_ext_time_t calculate_perf_combined(modsec_rec *msr) {
...
char *format_all_performance_variables(modsec_rec *msr, ngx_pool_t *mp) {
    return ngx_psprintf(mp,NGX_INT64_LEN*10+ngx_strlen("combined=, p1=, p2=, p3=, p4=, p5=, sr=, sw=, l=, gc=")+10,
		"combined=%T, p1=%T, p2=%T, p3=%T, p4=%T, p5=%T, sr=%T, sw=%T, l=%T, gc=%T",
		calculate_perf_combined(msr),
...
static int generate_performance_variable(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp,ngx_msec_t value)
{
    msre_var *rvar = NULL;

    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%T", value);
    rvar->value_len = ngx_strlen(rvar->value);
        ...
        ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    arr = ngx_table_elts(msr->perf_rules);
    te = (ngx_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        int match = 0;
        ...
        if (var->param == NULL) match = 1;
            ...
            if (var->param_data != NULL) { /* Regex. */
                ...
                if (!(msc_regexec((msc_regex_t *)var->param_data, te[i].key,
                                ngx_strlen(te[i].key), &my_error_msg) == PCRE_ERROR_NOMATCH)) match = 1;
            } else { /* Simple comparison. */
        ...
        if (match) {
            ...
            rvar->value = te[i].val;
            rvar->value_len = ngx_strlen(rvar->value);
            char *nname = log_escape_nq(mptmp, te[i].key);

			rvar->name = ngx_psprintf(mptmp, ngx_strlen("PERF_RULES:")+ngx_strlen(nname)+2,
				"PERF_RULES:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN+2,"%T",
        (ngx_ext_time_msec(ngx_ext_time_now())-msr->r->start_msec));
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp,NGX_INT64_LEN*7,
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN*2,"%02d%02d",
        (tm->tm_year / 100) + 19,
         tm->tm_year % 100);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%d", tm->tm_wday);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%02d", tm->tm_sec);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%02d", tm->tm_min);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%02d", tm->tm_hour);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%02d", tm->tm_mon + 1);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%02d", tm->tm_mday);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    tc = time(NULL);
    rvar = ngx_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = ngx_psprintf(mptmp, NGX_INT64_LEN+2,"%ld", (long)tc);
    rvar->value_len = ngx_strlen(rvar->value);
    ngx_table_addn(vartab, rvar->name, (void *)rvar);
...
static int var_request_basename_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = file_basename_str(mptmp,&msr->r->uri);
    return var_simple_generate(var, vartab, mptmp, value);
}
        ...
        msre_rule *rule, ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    request_line_length = ngx_strlen(msr->request_line) + /* \n\n: */ 2;
    full_request_length = request_line_length + headers_length +
        msr->msc_reqbody_length + /* \0: */1;

    full_request = (char*)malloc(sizeof(char)*full_request_length);
    ...
    ngx_snprintf((u_char*)full_request, request_line_length + 1,
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%d", msr->msc_full_request_length);
    return var_simple_generate(var, vartab, mptmp, value);
}
...
static int var_request_body_length_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%d", msr->msc_reqbody_length);
    return var_simple_generate(var, vartab, mptmp, value);
}
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    arr = ngx_table_elts(msr->matched_vars);
    te = (ngx_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        int match = 0;
        msc_string *str = (msc_string *)te[i].val;
        ...
        if (var->param == NULL) match = 1;
            ...
            if (var->param_data != NULL) { /* Regex. */
                char *my_error_msg = NULL;
                if (!(msc_regexec((msc_regex_t *)var->param_data, str->name,
                    ngx_strlen(str->name), &my_error_msg) == PCRE_ERROR_NOMATCH)) match = 1;
            } else { /* Simple comparison. */
                if (strcasecmp(str->name, var->param) == 0) match = 1;
            }
        }
        ...
        if (match && (strncmp(str->name,"MATCHED_VARS:",13) != 0) && (strncmp(str->name,"MATCHED_VARS_NAMES:",19))) {
            ...
            msre_var *rvar = ngx_palloc(mptmp, sizeof(msre_var));

            rvar->value = ngx_pstrmemdup(mptmp, str->name, ngx_strlen(str->name));
            rvar->value_len = ngx_strlen(rvar->value);
            rvar->name = ngx_pstrndup(mptmp,log_escape_nq(mptmp, str->name));
        ...
        ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match && (strncmp(str->name,"MATCHED_VARS:",13) != 0) && (strncmp(str->name,"MATCHED_VARS_NAMES:",19))) {
            ...
            msre_var *rvar = (msre_var*)ngx_palloc(mptmp, sizeof(msre_var));

            rvar->value = ngx_pstrmemdup(mptmp, str->value, str->value_len);
            rvar->value_len = str->value_len;

			char *nname = log_escape_nq(mptmp, str->name);

            rvar->name = ngx_pstrndup(mptmp,nname);
        ...
        ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    arr = ngx_table_elts(msr->request_cookies);
    te = (ngx_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        int match = 0;
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp, ngx_strlen("REQUEST_COOKIES:")+ngx_strlen(nname)+2,
				"REQUEST_COOKIES:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp, ngx_strlen(nname)+ngx_strlen("REQUEST_COOKIES_NAMES:")+2,
				"REQUEST_COOKIES_NAMES:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp,ngx_strlen(nname)+ngx_strlen("REQUEST_HEADERS:")+2,
				"REQUEST_HEADERS:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp,ngx_strlen(nname)+ngx_strlen("REQUEST_HEADERS_NAMES:")+2, 
				"REQUEST_HEADERS_NAMES:%s",nname);
...
static int var_request_filename_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    return var_simple_generate(var, vartab, mptmp, msr->parsed_uri.path);
}
...
static int var_server_port_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%u", msr->local_port);
    return var_simple_generate(var, vartab, mptmp, value);
}
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    if (ngx_http_map_uri_to_path(msr->r, &path, &root, 0) == NULL) {
        return 0;
    }

    char *value = file_basename_str(mptmp, &path);
    return var_simple_generate(var, vartab, mptmp, value);
}
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    return var_simple_generate(var, vartab, mptmp,(const char*)ngx_pstrdup(msr->mp,&path));
}
...
static int var_script_gid_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%ld", (long)msr->r->finfo.group);
    return var_simple_generate(var, vartab, mptmp, value);
}
...
{
    char *value = NULL;
    if (ngx_gid_name_get(&value, msr->r->finfo.group, mptmp) == ngx_SUCCESS) {
        return var_simple_generate(var, vartab, mptmp, value);
    }
    return 0;
}
...
static int var_script_mode_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = ngx_psprintf(mptmp, "%04x", msr->r->finfo.protection);
    return var_simple_generate(var, vartab, mptmp, value);
}
...
static int var_script_uid_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = ngx_psprintf(mptmp, "%ld", (long)msr->r->finfo.user);
    return var_simple_generate(var, vartab, mptmp, value);
}
...
{
    char *value = NULL;
    if (ngx_uid_name_get(&value, msr->r->finfo.user, mptmp) == ngx_SUCCESS) {
        return var_simple_generate(var, vartab, mptmp, value);
    }
    return 0;
}
...
static int var_auth_type_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    char *value = (char *)ngx_table_get(msr->request_headers, "Authorization");
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    arr = ngx_table_elts(msr->response_headers);
    te = (ngx_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        int match = 0;
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp,ngx_strlen(nname)+ngx_strlen("RESPONSE_HEADERS:")+2, 
				"RESPONSE_HEADERS:%s",nname);
    ...
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
			...
			rvar->name = ngx_psprintf(mptmp, ngx_strlen(nname)+ngx_strlen("RESPONSE_HEADERS_NAMES:")+2,
			"RESPONSE_HEADERS_NAMES:%s",nname);
...
static int var_response_status_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    const char *value = ngx_psprintf(mptmp, NGX_INT64_LEN,"%u", msr->response_status);
    return var_simple_generate(var, vartab, mptmp, value);
}
...
static int var_response_content_type(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
	return var_simple_generate(var, vartab, mptmp,msc_http_var_content_type(msr));
}
...
static int var_response_content_length(modsec_rec *msr, msre_var *var, msre_rule *rule,
    ngx_table_t *vartab, ngx_pool_t *mptmp)
{
    const char *value = ngx_psprintf(mptmp, NGX_INT64_LEN+2,
            "%ui",msc_http_var_content_length(msr,0));
    return var_simple_generate(var, vartab, mptmp, value);
}
...
{
    msre_var_metadata *metadata = (msre_var_metadata *)ngx_pcalloc(engine->mp,
    ...
    ngx_table_setn(engine->variables, name, (void *)metadata);
}
...
void msre_engine_register_default_variables(msre_engine *engine) {
    ...
    msre_engine_variable_register(engine,
        ...
        0, 0,
        NULL,
        var_sqlite_delete_error_generate,
        VAR_DONT_CACHE, /* dynamic */
        PHASE_REQUEST_BODY
    );
