...
CPTTree *CPTCreateRadixTree(ngx_pool_t *pool)   {
    ...
    tree = (CPTTree*)ngx_palloc(pool, sizeof(CPTTree));
...
TreeNode *CPTCreateNode(ngx_pool_t *pool)   {
    TreeNode *node = NULL;

    node = (TreeNode*)ngx_palloc(pool, sizeof(TreeNode));
...
CPTData *CPTCreateCPTData(unsigned char netmask, ngx_pool_t *pool) {

    CPTData *prefix_data = (CPTData*)ngx_palloc(pool, sizeof(CPTData));
...
TreePrefix *InsertDataPrefix(TreePrefix *prefix, unsigned char *ipdata, unsigned int ip_bitmask,
        unsigned char netmask, ngx_pool_t *pool)  {
...
TreePrefix *CPTCreatePrefix(unsigned char *ipdata, unsigned int ip_bitmask,
    ...
    prefix = (TreePrefix*)ngx_palloc(pool, sizeof(TreePrefix));
    if (prefix == NULL)
        return NULL;

    memset(prefix, 0, sizeof(TreePrefix));

    prefix->buffer = ngx_palloc(pool, bytes);
...
TreeNode *CPTCreateHead(TreePrefix *prefix, TreeNode *node, CPTTree *tree,unsigned char netmask, unsigned int ip_bitmask)    {
    ...
    if (node != NULL)   {
        ...
        node->count++;
        node->netmasks = ngx_palloc(tree->pool, (node->count *  sizeof(unsigned char)));
...
TreeNode *CPTAddElement(unsigned char *ipdata, unsigned int ip_bitmask, CPTTree *tree, unsigned char netmask)   {
    ...
    while (parent && test_bit <= (int)parent->bit) {
        node = parent;
        parent = node->parent;
    }
    ...
    if (test_bit == bitlen) {
        ...
        if (node->netmasks != NULL) {
            ...
            i_node->netmasks = ngx_palloc(tree->pool, (node->count - i) * sizeof(unsigned char));
    ...
    switch(type)    {
        ...
        case IPV4_TREE:
            ...
            *(ip_strv4 + (sizeof(ip_strv4) - 1)) = '\0';
            ...
            else if (netmask_v4 != NETMASK_32 && pos < (int)strlen(ip_strv4)) {
                ip_strv4[pos] = '\0';
            }
        ...
        case IPV6_TREE:
            ...
            *(ip_strv6 + sizeof(ip_strv6) - 1) = '\0';
            ...
            else if (netmask_v6 != NETMASK_128 && pos < (int)strlen(ip_strv6)) {
                ip_strv6[pos] = '\0';
            }
