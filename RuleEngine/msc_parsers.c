...
int parse_cookies_v0(modsec_rec *msr, char *_cookie_header,
                     ngx_table_t *cookies, const char *delim)
{
    ...
    if(msr->txcfg->cookiev0_separator == NULL) {
        p = ngx_strtok(cookie_header, delim, &saveptr);
    } else {
        p = ngx_strtok(cookie_header, msr->txcfg->cookiev0_separator, &saveptr);
    }
    ...
    while(p != NULL) {
        ...
        if ((attr_name != NULL)&&(strlen(attr_name) != 0)) {
            if (attr_value != NULL) {
                ...
                ngx_table_add(cookies, attr_name, attr_value);
            } else {
                if (msr->txcfg->debuglog_level >= 5) {
                ...
                ngx_table_add(cookies, attr_name, "");
            }
        ...
        if(msr->txcfg->cookiev0_separator == NULL) {
            p = ngx_strtok(NULL, delim, &saveptr);
        } else {
            p = ngx_strtok(NULL, msr->txcfg->cookiev0_separator, &saveptr);
        }
    }
        ...
        if ((attr_name != NULL)&&(strlen(attr_name) != 0)) {
            ...
            if (attr_name[0] == '$') {
                if (prev_attr_name != NULL) {
                     ...
                     */
                    attr_name = ngx_psprintf(msr->mp,strlen(prev_attr_name)+strlen(attr_name+1)+8, 
                            "$%s_%s", prev_attr_name, attr_name + 1);
                }
            }
...
int parse_arguments(modsec_rec *msr, const char *s, size_t inputlength,
        ...
        ngx_table_t *arguments, int *invalid_count)
{
    msc_arg *arg;
    size_t i, j;
    char *value = NULL;
    ...
    *invalid_count = 0;
    while (i < inputlength) {
        ...
        if (status == 0) {
            arg->name_len = urldecode_nonstrict_inplace_ex((u_char *)buf, arg->name_origin_len, invalid_count, &changed);
            arg->name = ngx_pstrmemdup(msr->mp, buf, arg->name_len);
            ...
            arg->value_len = urldecode_nonstrict_inplace_ex((u_char *)value, arg->value_origin_len, invalid_count, &changed);
            arg->value = ngx_pstrmemdup(msr->mp, value, arg->value_len);
...
void add_argument(modsec_rec *msr, ngx_table_t *arguments, msc_arg *arg)
{
    ...
    ngx_table_addn(arguments, log_escape_nq_ex(msr->mp, arg->name, arg->name_len), (void *)arg);
}
