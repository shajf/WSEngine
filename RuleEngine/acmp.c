...
#define utf8_lcase(a) ngx_tolower(a)
#endif
...
#ifdef ACMP_USE_UTF8
    return (parser->is_utf8 == 0) ? ngx_strlen(str) : utf8_strlen(str);
#else
    return ngx_strlen(str);
#endif
}
...
static void acmp_add_btree_leaves(acmp_btree_node_t *node, acmp_node_t *nodes[],
        int pos, int lb, int rb, ngx_pool_t *pool) {

    int left = 0, right = 0;
    if ((pos - lb) > 1) {
        left = lb + (pos - lb) / 2;
        node->left = ngx_pcalloc(pool, sizeof(acmp_btree_node_t));
    ...
    if ((rb - pos) > 1) {
        right = pos + (rb - pos) / 2;
        node->right = ngx_pcalloc(pool, sizeof(acmp_btree_node_t));
        /* ENH: Check alloc succeded */
...
static void acmp_build_binary_tree(ACMP *parser, acmp_node_t *node) {
    ...
    nodes = (acmp_node_t**)ngx_pcalloc(parser->pool, count * sizeof(acmp_node_t *));
    /* ENH: Check alloc succeded */
    ...
    node->btree = (acmp_btree_node_t*)ngx_pcalloc(parser->pool, sizeof(acmp_btree_node_t));
    /* ENH: Check alloc succeded */
    pos = count / 2;
 ...
 */
static ngx_int_t acmp_connect_fail_branches(ACMP *parser) {
    /* Already connected ? */
    acmp_node_t *child, *node, *goto_node;
    ngx_array_t *arr, *arr2, *tmp;

    if (parser->is_failtree_done != 0) return NGX_OK;

    parser->root_node->text = "";
    arr  = ngx_array_create(parser->pool, 32, sizeof(acmp_node_t *));
    arr2 = ngx_array_create(parser->pool, 32, sizeof(acmp_node_t *));
    ...
    for (child = parser->root_node->child; child != NULL; child = child->sibling) {
        ...
        *(acmp_node_t **)ngx_array_push(arr) = child;
#ifdef DEBUG_ACMP
    ...
    for (;;) {
        while (ngx_is_empty_array(arr) == 0) {
            node = *(acmp_node_t **)ngx_array_pop(arr);
            ...
            child = node->child;
            while (child != NULL) {
                *(acmp_node_t **)ngx_array_push(arr2) = child;
                child = child->sibling;
            }
        }
        if (ngx_is_empty_array(arr2) != 0) break;
 ...
 * pool  - ngx_pool to use as parent pool, can be set to NULL
 */
ACMP *acmp_create(int flags, ngx_pool_t *pool) {
    ACMP *parser;
    
    ngx_pool_t *p = ngx_create_pool(1024,pool->log);
    
    if(!p) return NULL;

    parser = (ACMP*)ngx_pcalloc(p, sizeof(ACMP));
    /* ENH: Check alloc succeded */
    parser->pool = p;
    ...
    parser->root_node = (acmp_node_t*)ngx_pcalloc(p, sizeof(acmp_node_t));
    /* ENH: Check alloc succeded */
    return parser;
}
 ...
 */
ngx_int_t acmp_add_pattern(ACMP *parser, const char *pattern,
        acmp_callback_t callback, void *data, size_t len)
{
    ...
    if (parser->is_active != 0) return NGX_ERROR;

    length = (len == 0) ? acmp_strlen(parser, pattern) : len;
    ucs_chars = (acmp_utf8_char_t*)ngx_pcalloc(parser->pool, length * sizeof(acmp_utf8_char_t));
    /* ENH: Check alloc succeded */
    ...
    for (i = 0; i < length; i++) {
        ...
        if (child == NULL) {
            child = (acmp_node_t*)ngx_pcalloc(parser->pool, sizeof(acmp_node_t));
            /* ENH: Check alloc succeded */
            child->pattern = "";
            child->letter = letter;
            child->depth = i;
            child->text = ngx_pcalloc(parser->pool, strlen(pattern) + 2);
            /* ENH: Check alloc succeded */
        ...
        if (i == length - 1) {
            if (child->is_last == 0) {
                parser->dict_count++;
                child->is_last = 1;
                child->pattern = ngx_pcalloc(parser->pool, strlen(pattern) + 2);
                /* ENH: Check alloc succeded */
 ...
 */
ngx_int_t acmp_process_quick(ACMPT *acmpt, const char **match, const char *data, size_t len) {
    ACMP *parser;
    ...
    while (data < end) {
        acmp_utf8_char_t letter = (char)*data++;
