...
int json_add_argument(modsec_rec *msr, const char *value, unsigned length)
{
    ...
    arg = (msc_arg *) ngx_pcalloc(msr->mp, sizeof(msc_arg));

    /**
     * Argument name is 'prefix + current_key'
     */
    if (msr->json->prefix) {
        arg->name = ngx_pstrcat(msr->mp, msr->json->prefix,".",msr->json->current_key,NULL);

    }
    else {
        arg->name = ngx_pstrndup(msr->mp,(const char*)msr->json->current_key);
    }
     ...
     */
    arg->value = ngx_pstrmemdup(msr->mp, value, length);
...
static int yajl_map_key(void *ctx, const unsigned char *key, size_t length)
{
     ...
     * in safe_key
     */
    safe_key = (unsigned char*)ngx_pstrmemdup(msr->mp, (const char*)key, length);
...
static int yajl_string(void *ctx, const unsigned char *value, size_t length)
{
    ...
    return json_add_argument(msr, (const char*)value, length);
}
...
static int yajl_start_map(void *ctx)
{
     ...
     */
    if (msr->json->prefix) {
        msr->json->prefix = (unsigned char*)ngx_pstrcat(msr->mp, msr->json->prefix,".",
            msr->json->current_key,NULL);
    }
    else {
        msr->json->prefix = (unsigned char*)ngx_pstrndup(msr->mp, (const char*)msr->json->current_key);
    }
...
static int yajl_end_map(void *ctx)
{
     ...
     */
    separator = (unsigned char*)strrchr((const char*)msr->json->prefix, '.');

    if (separator) {
        msr->json->prefix = (unsigned char*)ngx_pstrmemdup(msr->mp, (const char*)msr->json->prefix,
            separator - msr->json->prefix);
        msr->json->current_key = (unsigned char*)ngx_pstrndup(msr->mp, (const char*)separator + 1);
    }
    else {
        /**
...
int json_init(modsec_rec *msr, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    msr->json = (json_data*)ngx_pcalloc(msr->mp, sizeof(json_data));
...
int json_process_chunk(modsec_rec *msr, const char *buf, unsigned int size, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    msr->json->status = yajl_parse(msr->json->handle, (const unsigned char*)buf, size);
    if (msr->json->status != yajl_status_ok) {
        /* We need to free the yajl error message later, how to do this? */
        *error_msg = (char*)yajl_get_error(msr->json->handle, 0, (const unsigned char*)buf, size);
        return -1;
    }
...
int json_complete(modsec_rec *msr, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (msr->json->status != yajl_status_ok) {
        /* We need to free the yajl error message later, how to do this? */
        *error_msg = (char*)yajl_get_error(msr->json->handle, 0, NULL, 0);
        return -1;
    }
