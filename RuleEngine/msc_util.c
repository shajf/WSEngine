...
char *utf8_unicode_inplace_ex(ngx_pool_t *mp, unsigned char *input, long int input_len, int *changed) {
    unsigned int unicode_len = 0, length = 0;
        ...
        /* If first byte begins with binary 110 it is two byte encoding*/
            ...
            /* check second byte starts with binary 10 */
                ...
                if(count <= len) {
                    ...
                    *data++ = '%';
                    *data++ = 'u';
                    unicode = (unsigned char*)ngx_psprintf(mp, NGX_INT64_LEN*2+2,"%xud", d);
                    length = ngx_strlen(unicode);
...
char *parse_pm_content(const char *op_parm, unsigned short int op_len, msre_rule *rule, char **error_msg)  {
    ...
    content = ngx_pstrndup(rule->ruleset->mp,op_parm);

    if (content == NULL) {
        *error_msg = ngx_pstrndup(rule->ruleset->mp,"Error allocating memory for pattern matching content.");

        return NULL;
    }

    while (offset < op_len && ngx_isspace(content[offset])) {
        offset++;
    };

    op_len = ngx_strlen(content);

    if (content[offset] == '\"' && content[op_len-1] == '\"') {
        parm = ngx_pstrndup(rule->ruleset->mp, content + offset + 1);
        if (parm  == NULL) {
            *error_msg = ngx_pstrndup(rule->ruleset->mp,"Error allocating memory for pattern matching content.");
            return NULL;
        }
        ...
        parm = ngx_pstrndup(rule->ruleset->mp, content + offset);
        if (parm == NULL) {
            *error_msg = ngx_pstrndup(rule->ruleset->mp,"Error allocating memory for pattern matching content.");
            return NULL;
        }
    ...
    op_len = ngx_strlen(parm);

    if (op_len == 0)   {
        *error_msg = ngx_pstrndup(rule->ruleset->mp,"Content length is 0.");
        return NULL;
    }
        ...
        if (parm[i] == '|') {
            ...
            if (bin) {
                        ...
                        parm[i] == '\"')
                {
                    ...
                    *error_msg = ngx_pstrndup(rule->ruleset->mp,"Unsupported escape sequence.");
                    return NULL;
                }
                esc = 0;
                converted = 1;
    ...
    processed = ngx_pstrmemdup(rule->ruleset->mp, parm, op_len);
...
char *remove_quotes(ngx_pool_t *mptmp, const char *input, int input_len)  {
    char *parm = (char*)ngx_palloc(mptmp, input_len);
    char *ret = parm;
    int len = input_len;
...
int set_match_to_tx(modsec_rec *msr, int capture, const char *match, int tx_n)  {
    ...
    if (capture) {
        ...
        s->name = ngx_psprintf(msr->mp,NGX_INT64_LEN+2,"%d", tx_n);
...
int parse_name_eq_value(ngx_pool_t *mp, const char *input, char **name, char **value) {
    ...
    *name = ngx_pstrmemdup(mp, input, p - input);
    if (*name == NULL) return -1;
    p++;

    *value = ngx_pstrndup(mp, p);
 ...
 *
 */
char *file_basename(ngx_pool_t *mp, const char *filename) {
    char *d, *p;

    if (filename == NULL) return NULL;
    d = ngx_pstrndup(mp, filename);
...
char  *file_basename_str(ngx_pool_t *mp, ngx_str_t *filename){

    char *d, *p;

    if (filename == NULL||filename->data== NULL) return NULL;
    d = (char*)ngx_pstrdup(mp, filename);
...
char *guess_tmp_dir(ngx_pool_t *p) {
    ...
    /* ENH Use ngx_temp_dir_get instead. */
...
char *current_logtime(ngx_pool_t *mp) {
    ngx_ext_time_exp_t t;
    char tstr[100];
    size_t len;

    ngx_ext_time_exp_lt(&t, ngx_ext_time_now());

    ngx_ext_strftime(tstr, &len, 80, "%d/%b/%Y:%H:%M:%S ", &t);
    
    ngx_snprintf((u_char*)tstr + strlen(tstr), 80 - ngx_strlen(tstr), "%c%.2d%.2d",
            t.tm_gmtoff < 0 ? '-' : '+',
            t.tm_gmtoff / (60 * 60), (t.tm_gmtoff / 60) % 60);
    return ngx_pstrndup(mp, tstr);
}
...
char *current_filetime(ngx_pool_t *mp) {
    ...
    ngx_ext_strftime(tstr, &len, 80, "%Y%m%d-%H%M%S", &t);
    return ngx_pstrndup(mp, tstr);
}
...
int msc_mkstemp_ex(char *templat, int mode) {
    ...
    /* ENH Use ngx_file_mktemp instead. */

    if (mktemp((char*)templat) == NULL) return -1;
    fd = open(templat, O_WRONLY | O_APPEND | O_CREAT, mode);
...
static char *_log_escape(ngx_pool_t *p, const unsigned char *input,
        unsigned long int input_length, int escape_quotes, int escape_colon, int escape_re);

char *log_escape_re(ngx_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char *)text, text ? ngx_strlen(text) : 0, 1, 1, 1);
}

char *log_escape(ngx_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char*)text, text ? ngx_strlen(text) : 0, 1, 0, 0);
}

char *log_escape_nq(ngx_pool_t *mp, const char *text) {
    return _log_escape(mp, (const unsigned char*)text, text ? ngx_strlen(text) : 0, 0, 0, 0);
}

char *log_escape_ex(ngx_pool_t *mp, const char *text, unsigned long int text_length) {
    return _log_escape(mp, (const unsigned char*)text, text_length, 1, 0, 0);
}

char *log_escape_nq_ex(ngx_pool_t *mp, const char *text, unsigned long int text_length) {
...
char *log_escape_raw(ngx_pool_t *mp, const unsigned char *text, unsigned long int text_length) {
    unsigned char *ret = ngx_palloc(mp, text_length * 4 + 1);
 ...
 */
static char *_log_escape(ngx_pool_t *mp, const unsigned char *input, unsigned long int input_len,
        int escape_quotes, int escape_colon, int escape_re)
{
    ...
    ret = ngx_palloc(mp, input_len * 4 + 1);
 ...
 * IMP1 Assumes NUL-terminated
 */
int html_entities_decode_inplace(ngx_pool_t *mp, unsigned char *input, int input_len) {
    unsigned char *d = input;
    int i, count;
            ...
            if (input[j] == '#') {
                ...
                if ((input[j] == 'x')||(input[j] == 'X')) {
                    ...
                    if (j > k) { /* Do we have at least one digit? */
                        /* Decode the entity. */
                        char *x = ngx_pstrmemdup(mp, (const char*)&input[k], j - k);
...
char *resolve_relative_path(ngx_pool_t *pool, const char *parent_filename, const char *filename) {
    ...
    return ngx_pstrcat(pool, ngx_pstrmemdup(pool, parent_filename,
                ngx_strlen(parent_filename) - ngx_strlen(ngx_filepath_name_get(parent_filename))),
            filename, NULL);
}
...
char *construct_single_var(modsec_rec *msr, char *name) {
    ...
    varname = ngx_pstrndup(msr->mp, name);
...
/**
 * @brief Transforms an ngx_array_t to a text buffer
 *
 * Converts an ngx_array_t into a plain/text buffer in a Key: Pair
 ...
 *
 * @param arr ngx_array_t to be iterated.
 ...
 *
 */
int msc_headers_to_buffer(const ngx_array_t *arr, char *buffer,
        int buffer_length)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
            char *value = te[i].val;
            char *key = te[i].key;
            headers_length = headers_length + ngx_strlen(value) + ngx_strlen(key) + /* \n: */ 1 +
               /* colum */ 1 + /* space: */ 1 ;
...
int create_radix_tree(ngx_pool_t *mp, TreeRoot **rtree, char **error_msg)
{
    *rtree = (TreeRoot*)ngx_palloc(mp, sizeof(TreeRoot));
    if (*rtree == NULL)
    {
        *error_msg = ngx_pstrndup(mp,"Failed allocating memory to TreeRoot.");
    ...
    {
        *error_msg = ngx_pstrndup(mp,"IPmatch: Tree initialization failed.");
    ...
    ngx_pool_t *mp, char **error_msg)
{
    ...
    file =  ngx_open_file_read(mp,uri);

    if (file == NULL)
    {
        *error_msg = ngx_pstrcat(mp,"Could not open ipmatch file:",uri,NULL);
        return -1;
    }

    while ((rc = ngx_file_gets(buf, HUGE_STRING_LEN, file)) != NGX_EOF)
    {
        line++;
        if (rc != NGX_OK)
        {
            *error_msg = ngx_psprintf(mp, ngx_strlen("Could not read \"\" line ")+ngx_strlen(uri)+NGX_INT64_LEN+2,
                    "Could not read \"%s\" line %d",uri, line);
        ...
        while ((ngx_isspace(*start) != 0) && (*start != '\0'))
        {
            start++;
        }
        ...
        for (end = start; end != NULL || *end != '\0' || *end != '\n'; end++)
        {
            ...
            if (*end != '\n')
            {
                *error_msg = ngx_psprintf(mp, ngx_strlen("Invalid char \"\" in line  of file ")+ngx_strlen(uri)+NGX_INT64_LEN+2,
                        "Invalid char \"%c\" in line %d of file %s", *end, line, uri);
            }
        ...
        *end = '\0';
        ...
        if (tnode == NULL)
        {
            *error_msg = ngx_psprintf(mp, ngx_strlen("Could not add entry \"\" in line  of file  to IP list")+ngx_strlen(start)+ngx_strlen(uri)+NGX_INT64_LEN+2,
                    "Could not add entry \"%s\" in line %d of file %s to IP list", start, line, uri);
            return -1;
        }
    }
    ...
    const char *value, modsec_rec *msr, char **error_msg)
{
    ...
    if (strchr(value, ':') == NULL) {
        if (inet_pton(AF_INET, value, &in) <= 0) {
            *error_msg = ngx_pstrcat(mp, "IPmatch: bad IPv4 \"specification \"",value,"\".",NULL);
            return -1;
        }
        ...
        if (inet_pton(AF_INET6, value, &in6) <= 0) {
            *error_msg = ngx_pstrcat(mp, "IPmatch: bad IPv6 \"specification \"",value,"\".",NULL);
            return -1;
        }
...
int ip_tree_from_param(ngx_pool_t *mp,
    char *param, TreeRoot **rtree, char **error_msg)
{
    ...
    while (str != NULL)
    {
        ...
        if (tnode == NULL)
        {
            *error_msg = ngx_pstrcat(mp, "Could not add entry \"",str,"\" from: ", param,NULL);
            return -1;
        }

        str = ngx_strtok(NULL, ",", &saved);
    }
...
int ip_check(const char* ip){
    ...
    ipn = strtol(p,&stop,10);

    if(stop==NULL||*stop!='.'||stop==p||ipn<0||ipn>255||stop==ip+len-1) return 0;
    ...
    if(stop!=ip+len||ipn<0||ipn>255) return 0;
...
int generate_random_bytes(char *buf,size_t length){
 int fd = -1;
     ...
     * gives EOF, so reading 'length' bytes may require pening the
...
{
    ngx_regfree((ngx_pcre_t *) preg);
}

ngx_pcre_t * ngx_pregcomp(ngx_pool_t *p, const char *pattern,
                                     int cflags)
{
    ngx_pool_cleanup_t *clp = NULL;
    
    ngx_pcre_t *preg = (ngx_pcre_t*)ngx_palloc(p, sizeof(ngx_pcre_t));

    if (ngx_regcomp(preg, pattern, cflags)) {
        return NULL;
    }

    clp = ngx_pool_cleanup_add(p,0);
