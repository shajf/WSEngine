...
static int multipart_parse_content_disposition(modsec_rec *msr, char *c_d_value) {
    ...
    while(*p != '\0') {
        ...
        name = ngx_pstrmemdup(msr->mp, start, (p - start));
        ...
        if ((*p == '"') || (*p == '\'')) {
            ...
            value = ngx_pstrmemdup(msr->mp, start, (p - start));
        }
        ...
        if (strcmp(name, "name") == 0) {
            ...
            msr->multipart_name = ngx_pstrndup(msr->mp, value);
        ...
        if (strcmp(name, "filename") == 0) {
            ...
            msr->multipart_filename = ngx_pstrndup(msr->mp, value);
...
static int multipart_process_part_header(modsec_rec *msr, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    for(i = 0; i < len; i++) {
        if (msr->mpd->buf[i] == '\0') {
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: Nul byte in part headers.");
            return -1;
        }
    }
    ...
    { /* Empty line. */
        ...
        header_value = (char *)ngx_table_get(msr->mpd->mpp->headers, "Content-Disposition");
        if (header_value == NULL) {
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: Part missing Content-Disposition header.");
            return -1;
        }

        rc = multipart_parse_content_disposition(msr, header_value);
        if (rc < 0) {
            char *nname = log_escape_nq(msr->mp, header_value);

            *error_msg = ngx_psprintf(msr->mp, 
                    ngx_strlen("Multipart: Invalid Content-Disposition header (%d): %s.")+ngx_strlen(nname)+NGX_INT64_LEN+2,
                    "Multipart: Invalid Content-Disposition header (%d): %s.",
                    rc, nname);
        ...
        if (msr->mpd->mpp->name == NULL) {
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: Content-Disposition header missing name field.");
            return -1;
        }
        ...
        if (msr->mpd->mpp->filename != NULL) {
            ...
            if (strstr(header_value, "filename=") == NULL) {
                *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid Content-Disposition header (filename).");
                return -1;
            }
        ...
        if (isspace(msr->mpd->buf[0])) {
            ...
            if (msr->mpd->mpp->last_header_name == NULL) {
                ...
                *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid part header (folding error).");
                return -1;
            }
            ...
            new_value = ngx_pstrndup(msr->mp, data);
            remove_lf_crlf_inplace(new_value);

            /* update the header value in the table */
            header_value = (char *)ngx_table_get(msr->mpd->mpp->headers, msr->mpd->mpp->last_header_name);
            new_value = ngx_pstrcat(msr->mp, header_value, " ", new_value, NULL);
            ngx_table_set(msr->mpd->mpp->headers, msr->mpd->mpp->last_header_name, new_value);
            ...
            if (strlen(new_value) > MULTIPART_BUF_SIZE) {
                *error_msg = ngx_pstrndup(msr->mp, "Multipart: Part header too long.");
                return -1;
            }
        } else {
            ...
            if (*data == '\0') {
                *error_msg = ngx_pstrcat(msr->mp, "Multipart: Invalid part header (colon missing): ",
                    log_escape_nq(msr->mp, msr->mpd->buf),NULL);

                return -1;
            }

            /* extract header name */
            header_name = ngx_pstrmemdup(msr->mp, msr->mpd->buf, (data - msr->mpd->buf));
            if (data == msr->mpd->buf) {
                *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid part header (header name missing).");
            ...
            header_value = ngx_pstrndup(msr->mp, data);
            remove_lf_crlf_inplace(header_value);

            /* error if the name already exists */
            if (ngx_table_get(msr->mpd->mpp->headers, header_name) != NULL) {
                *error_msg = ngx_pstrcat(msr->mp, "Multipart: Duplicate part header: ",
                    log_escape_nq(msr->mp, header_name),NULL);

                return -1;
            }

            ngx_table_setn(msr->mpd->mpp->headers, header_name, header_value);
    ...
    if (msr->mpd->mpp->type == MULTIPART_FILE) {
        ...
        if (extract && (msr->mpd->nfiles >= msr->txcfg->upload_file_limit)) {
            if (msr->mpd->flag_file_limit_exceeded == 0) {
                *error_msg = ngx_psprintf(msr->mp,
                            ngx_strlen("Multipart: Upload file limit exceeded SecUploadFileLimit %d.")+NGX_INT64_LEN+2,
                            "Multipart: Upload file limit exceeded SecUploadFileLimit %d.",
        ...
        if (extract) {
            ...
            if (msr->mpd->mpp->tmp_file_fd == 0) {
                ...
                msr->mpd->mpp->tmp_file_name = ngx_pstrcat(msr->mp,msr->txcfg->tmp_dir,
                        "/",current_filetime(msr->mp),"-",msr->txid,"-file-XXXXXX",NULL);

                msr->mpd->mpp->tmp_file_fd = msc_mkstemp_ex(msr->mpd->mpp->tmp_file_name, CREATEMODE_UNISTD);

                /* do we have an opened file? */
                if (msr->mpd->mpp->tmp_file_fd < 0) {
                    *error_msg = ngx_pstrcat(msr->mp, "Multipart: Failed to create file: ",
                        log_escape_nq(msr->mp, msr->mpd->mpp->tmp_file_name),NULL);
                    return -1;
                }
                ...
                if (msr->txcfg->debuglog_level >= 4) {
                    msr_log(msr, 4,
                        "Multipart: Created temporary file %d : %s",
                        msr->mpd->nfiles,
            ...
            if (msr->mpd->reserve[0] != 0) {
                if (write(msr->mpd->mpp->tmp_file_fd, &msr->mpd->reserve[1], msr->mpd->reserve[0]) != msr->mpd->reserve[0]) {
                    *error_msg = ngx_pstrcat(msr->mp, 
                            "Multipart: writing to \"",log_escape(msr->mp, msr->mpd->mpp->tmp_file_name),"\" failed",NULL);
        ...
        value_part_t *value_part = (value_part_t*)ngx_pcalloc(msr->mp, sizeof(value_part_t));
        ...
        if (msr->mpd->reserve[0] != 0) {
            value_part->data = ngx_palloc(msr->mp, (MULTIPART_BUF_SIZE - msr->mpd->bufleft) + msr->mpd->reserve[0]);
            ...
            value_part->data = ngx_pstrmemdup(msr->mp, msr->mpd->buf, value_part->length);
            msr->mpd->mpp->length += value_part->length;
        }

        *(value_part_t **)ngx_array_push(msr->mpd->mpp->value_parts) = value_part;
        ...
        *error_msg = ngx_psprintf(msr->mp, 
                ngx_strlen("Multipart: unknown part type %d")+NGX_INT64_LEN+2,
                "Multipart: unknown part type %d", msr->mpd->mpp->type);
 ...
 *
 */
static char *multipart_combine_value_parts(modsec_rec *msr, ngx_array_t *value_parts) {
    value_part_t **parts = NULL;
    char *rval = (char*)ngx_palloc(msr->mp, msr->mpd->mpp->length + 1);
...
static int multipart_process_boundary(modsec_rec *msr, int last_part, char **error_log) {
    ...
    if (msr->mpd->mpp != NULL) {
        ...
        if (msr->mpd->mpp->name) {
            ...
            *(multipart_part **)ngx_array_push(msr->mpd->parts) = msr->mpd->mpp;
            if (msr->mpd->mpp->type == MULTIPART_FILE) {
                if (msr->txcfg->debuglog_level >= 9) {
    ...
    if (last_part == 0) {
        ...
        msr->mpd->mpp = (multipart_part *)ngx_pcalloc(msr->mp, sizeof(multipart_part));
        if (msr->mpd->mpp == NULL) return -1;
        msr->mpd->mpp->type = MULTIPART_FORMDATA;
        msr->mpd->mpp_state = 0;

        msr->mpd->mpp->headers = ngx_table_make(msr->mp, 10);
        ...
        msr->mpd->mpp->value_parts = ngx_array_create(msr->mp, 10, sizeof(value_part_t *));
    }
...
static int multipart_count_boundary_params(ngx_pool_t *mp, const char *header_value) {
    char *duplicate = NULL;
    char *s = NULL;
    int count = 0;

    if (header_value == NULL) return -1;
    duplicate = ngx_pstrndup(mp, header_value);
...
int multipart_init(modsec_rec *msr, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    msr->mpd = (multipart_data *)ngx_pcalloc(msr->mp, sizeof(multipart_data));
    if (msr->mpd == NULL) return -1;

    msr->mpd->parts = ngx_array_create(msr->mp, 10, sizeof(multipart_part *));
    ...
    if (msr->request_content_type == NULL) {
        ...
        *error_msg = ngx_pstrndup(msr->mp, "Multipart: Content-Type header not available.");
        return -1;
    }

    if (strlen(msr->request_content_type) > 1024) {
        msr->mpd->flag_error = 1;
        *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (length).");
        return -1;
    }

    if (strncasecmp(msr->request_content_type, "multipart/form-data", 19) != 0) {
        msr->mpd->flag_error = 1;
        *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid MIME type.");
        return -1;
    }
    ...
    if (multipart_count_boundary_params(msr->mp, msr->request_content_type) > 1) {
        msr->mpd->flag_error = 1;
        *error_msg = ngx_pstrndup(msr->mp, "Multipart: Multiple boundary parameters in C-T.");
        return -1;
    }
    ...
    if (msr->mpd->boundary != NULL) {
        ...
        for (p = (char *)(msr->request_content_type + 19); p < msr->mpd->boundary; p++) {
            if (!isspace(*p)) {
                if ((seen_semicolon == 0) && (*p == ';')) {
                    ...
                    msr->mpd->flag_error = 1;
                    *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (malformed).");
                    return -1;
                }
            }
        }
        ...
        if (b != (msr->mpd->boundary + 8)) {
            ...
            for (p = msr->mpd->boundary + 8; p < b; p++) {
                if (isspace(*p)) {
                    ...
                    msr->mpd->flag_error = 1;
                    *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (parameter name).");
                    return -1;
                }
            }
        }
        ...
        if ((len >= 2) && (*b == '"') && (*(b + len - 1) == '"')) {
            /* Quoted. */
            msr->mpd->boundary = ngx_pstrmemdup(msr->mp, b + 1, len - 2);
            ...
            {
                msr->mpd->flag_error = 1;
                *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (quote).");
                return -1;
            }

            msr->mpd->boundary = ngx_pstrndup(msr->mp, b);
            if (msr->mpd->boundary == NULL) return -1;
        ...
        if (multipart_count_boundary_params(msr->mp, msr->mpd->boundary) != 0) {
            msr->mpd->flag_error = 1;
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (content).");
            return -1;
        }
        ...
        if (multipart_boundary_characters_valid(msr->mpd->boundary) != 1) {
            msr->mpd->flag_error = 1;
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (characters).");
            return -1;
        }
        ...
        if (strlen(msr->mpd->boundary) == 0) {
            msr->mpd->flag_error = 1;
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (empty).");
            return -1;
        }
    }
        ...
        if (multipart_count_boundary_params(msr->mp, msr->request_content_type) > 0) {
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary in C-T (case sensitivity).");
            return -1;
        }

        *error_msg = ngx_pstrndup(msr->mp, "Multipart: Boundary not found in C-T.");
        return -1;
    }
    ...
    if ((msr->mpd->seen_data != 0) && (msr->mpd->is_complete == 0)) {
        if (msr->mpd->boundary_count > 0) {
            ...
            if (msr->mpd->is_complete == 0) {
                *error_msg = ngx_pstrndup(msr->mp, "Multipart: Final boundary missing.");
                return -1;
            }
        } else {
            *error_msg = ngx_pstrndup(msr->mp, "Multipart: No boundaries found in payload.");
            return -1;
        }
    }
                ...
                && (*(msr->mpd->buf + 1) == '-') )
            {
                    ...
                    && (strncmp(msr->mpd->buf + 2, msr->mpd->boundary, strlen(msr->mpd->boundary)) == 0) )
                {
                    ...
                    if (   (msr->mpd->flag_boundary_quoted)
                        ...
                        msr->mpd->flag_error = 1;
                        *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary (quotes).");
                        return -1;
                    }
                    ...
                    if (   (p != msr->mpd->buf + 2)
                        ...
                        msr->mpd->flag_error = 1;
                        *error_msg = ngx_pstrndup(msr->mp, "Multipart: Invalid boundary (whitespace).");
                        return -1;
                    }
                ...
                if (msr->mpd->mpp == NULL) {
                    ...
                    if (msr->mpd->mpp_state == 0) {
                        if ((msr->mpd->bufleft == 0) || (process_buffer)) {
                            ...
                            msr->mpd->flag_error = 1;
                            *error_msg = ngx_psprintf(msr->mp,
                                    ngx_strlen("Multipart: Part header line over %d bytes long")+NGX_INT64_LEN+2,
 ...
 *
 */
ngx_int_t multipart_cleanup(modsec_rec *msr) {
    int keep_files = 0;
        ...
        for(i = 0; i < msr->mpd->parts->nelts; i++) {
                 ...
                 && (parts[i]->tmp_file_size == 0))
            {
                ...
                if (parts[i]->tmp_file_name != NULL) {
                    ...
                    new_filename = ngx_pstrcat(msr->mp,msr->txcfg->upload_dir, "/",new_basename,NULL);
                    if (new_filename == NULL) return -1;

                    if (ngx_rename_file(parts[i]->tmp_file_name, new_filename) != 0)
                    {
 ...
 *
 */
int multipart_get_arguments(modsec_rec *msr, char *origin, ngx_table_t *arguments) {
    multipart_part **parts;
    ngx_uint_t i;
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if (parts[i]->type == MULTIPART_FORMDATA) {
            msc_arg *arg = (msc_arg *)ngx_pcalloc(msr->mp, sizeof(msc_arg));
...
char *multipart_reconstruct_urlencoded_body_sanitize(modsec_rec *msr) {
    ...
    /* allocate the buffer */
    body = (char*)ngx_palloc(msr->mp, body_len + 1);
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if (parts[i]->type == MULTIPART_FORMDATA) {
            ...
            if (msr->phase >= PHASE_LOGGING) {
                if (ngx_table_get(msr->arguments_to_sanitize, parts[i]->name) != NULL) {
