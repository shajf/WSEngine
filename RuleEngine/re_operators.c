...
{
    msre_op_metadata *metadata = (msre_op_metadata *)ngx_pcalloc(engine->mp,
    ...
    ngx_table_setn(engine->operators, name, (void *)metadata);
}
...
msre_op_metadata *msre_engine_op_resolve(msre_engine *engine, const char *name) {
    return (msre_op_metadata *)ngx_table_get(engine->operators, name);
}
...
static int msre_op_ipmatch_param_init(msre_rule *rule, char **error_msg) {
    ...
    param = ngx_pstrndup(rule->ruleset->mp, rule->op_param);
...
static int msre_op_ipmatch_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    if (res > 0) {
        *error_msg = ngx_pstrcat(msr->mp,
                "IPmatch: \"",var->value,"\" matched at ",var->name,NULL);
    }
...
static int msre_op_ipmatchFromFile_param_init(msre_rule *rule, char **error_msg) {
    ...
    if ((rule->op_param == NULL) || (ngx_strlen(rule->op_param) == 0))
    {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Missing parameter for " \
            "operator 'ipmatchFromFile'.");
        return 0;
    }

    fn = ngx_pstrndup(rule->ruleset->mp, rule->op_param);
    while ((ngx_isspace(*fn) != 0) && (*fn != '\0'))
    {
        fn++;
    }
    if (*fn == '\0') {
        *error_msg = ngx_pstrcat(rule->ruleset->mp, 
                "Empty file specification for operator ipmatchFromFile \"",fn,"\"", NULL);
        return 0;
    }

    filepath = fn;

    ipfile_path = ngx_pstrmemdup(rule->ruleset->mp, rule->filename,
        ngx_strlen(rule->filename) - ngx_strlen(ngx_filepath_name_get(rule->filename)));

    if (ngx_filepath_root(&rootpath, &filepath, NGX_FILEPATH_TRUENAME,
        rule->ruleset->mp) != NGX_OK) {
        ngx_filepath_merge(&fn, ipfile_path, fn, NGX_FILEPATH_TRUENAME, rule->ruleset->mp);
    }
...
static int msre_op_ipmatchFromFile_execute(modsec_rec *msr, msre_rule *rule,
    ...
    if (res > 0)
        *error_msg = ngx_pstrcat(msr->mp, "IPmatchFromFile: \"",var->value,"\" matched at",var->name,NULL);
...
static char *param_remove_escape(msre_rule *rule, char *str, int len)  {
    char *parm = (char*)ngx_pcalloc(rule->ruleset->mp, len);
    char *ret = parm;
...
static int msre_op_rsub_param_init(msre_rule *rule, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (ngx_tolower(*line) != 's') {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Error rsub operator format, must be s/ pattern");
        return -1;
    }

    data = ngx_pstrndup(rule->ruleset->mp, line);
    delim = *++data;
    if (delim)
        reg_pattern = ++data;
    if (reg_pattern) {
    ...
    if (!delim || !reg_pattern || !replace) {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Error rsub operator format - must be s/regex/str/[flags]");
        return -1;
    }

    op_len = ngx_strlen(replace);
    parsed_replace = ngx_pstrndup(rule->ruleset->mp, parse_pm_content(param_remove_escape(rule, replace, ngx_strlen(replace)),
        op_len, rule, error_msg));

    if(!parsed_replace) {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Error rsub operator parsing input data");
        return -1;
    }

    rule->sub_str = ngx_pstrmemdup(rule->ruleset->mp, parsed_replace, ngx_strlen(parsed_replace));

    if (flags) {
        while (*flags) {
            delim = ngx_tolower(*flags);
            if (delim == 'i')
                ignore_case = 1;
            else if (delim == 'd')
                rule->escape_re = 1;
            else
                *error_msg = ngx_pstrndup(rule->ruleset->mp, "Regex flag not supported");
            flags++;
        }
    }

    e_pattern = param_remove_escape(rule, reg_pattern, ngx_strlen(reg_pattern));
    pattern = ngx_pstrmemdup(rule->ruleset->mp, e_pattern, ngx_strlen(e_pattern));

    if(strstr(pattern,"%{") == NULL)    {
        regex = ngx_pregcomp(rule->ruleset->mp, pattern, NGX_PCRE_EXTENDED |
                (ignore_case ? NGX_PCRE_ICASE : 0));
        rule->sub_regex = regex;
    } else {
        rule->re_precomp = 1;
        rule->re_str = ngx_pstrmemdup(rule->ruleset->mp, pattern, strlen(pattern));
        rule->sub_regex = NULL;
    }
...
static int msre_op_rsub_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    msc_string *str = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    msc_string *re_pattern = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    char *offset = NULL;
    ...
    ngx_regmatch_t pmatch[NGX_MAX_REG_MATCH];

    if (error_msg == NULL) return -1;
    *error_msg = NULL;

    if(ngx_strcmp(var->name,"STREAM_OUTPUT_BODY") == 0 )  {
        output_body = 1;
    } else if(ngx_strcmp(var->name,"STREAM_INPUT_BODY") == 0 )  {
        input_body = 1;
    } else  {
    ...
    if(rule->re_precomp == 1)    {
        re_pattern->value = ngx_pstrndup(msr->mp, rule->re_str);
        re_pattern->value_len = ngx_strlen(re_pattern->value);

        expand_macros(msr, re_pattern, rule, msr->mp);

        if(ngx_strlen(re_pattern->value) > 0)   {
            if(rule->escape_re == 1)    {
                pattern = log_escape_re(msr->mp, re_pattern->value);
                if (msr->txcfg->debuglog_level >= 6) {
                    msr_log(msr, 6, "Escaping pattern [%s]",pattern);
                }
                rule->sub_regex = ngx_pregcomp(msr->mp, pattern, NGX_PCRE_EXTENDED);
            } else  {
                rule->sub_regex = ngx_pregcomp(msr->mp, re_pattern->value, NGX_PCRE_EXTENDED);
            }
        } else    {
            rule->sub_regex = NULL;
        }
    ...
    str->value = ngx_pstrndup(msr->mp, rule->sub_str);
    str->value_len = ngx_strlen(str->value);

    if(ngx_strstr(rule->sub_str,"%{") != NULL)
        expand_macros(msr, str, rule, msr->mp);

    maxsize=var->value_len+(NGX_MAX_REG_MATCH*1024)+1;

nextround:

    data = (char*)ngx_pcalloc(msr->mp, maxsize+1);
    ...
    data_out=data;
    size=0;
    for (offset = (char*)var->value; !ngx_regexec(rule->sub_regex,  offset, NGX_MAX_REG_MATCH, pmatch, 0); )  {
    ...
    *data_out=0;
    ...
    if (! *error_msg) {
        *error_msg = ngx_pstrndup(msr->mp, "Operator rsub succeeded.");
    }
...
static int msre_op_validateHash_param_init(msre_rule *rule, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if(ngx_strstr(pattern,"%{") == NULL)    {
        ...
        if (regex == NULL) {
            *error_msg = ngx_psprintf(rule->ruleset->mp, 
                    ngx_strlen("Error compiling pattern (offset %d): %s")+ngx_strlen(errptr)+NGX_INT64_LEN+2,
                    "Error compiling pattern (offset %d): %s",
                    erroffset, errptr);
            return 0;
        }
        ...
        if ((rc != 0) || (jit != 1)) {
            *error_msg = ngx_psprintf(rule->ruleset->mp,128,
                    "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                    "Execution error - "
                    "Does not support JIT (%d)",
                    rule,((rule->actionset != NULL)&&((rule->actionset->id != NULL)&&
                            (rule->actionset->id != NGX_CONF_UNSET_PTR))) ? rule->actionset->id : "-",
...
static int msre_op_validateHash_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    const char *rid_str = ((rule->actionset != NULL)&&((rule->actionset->id != NULL)&&(rule->actionset->id != NGX_CONF_UNSET_PTR))) ? rule->actionset->id : (const char*)"-";
    const char *fname_str =  rule->filename != NULL ? rule->filename : (const char*)"-";
    size_t rid_str_len = ngx_strlen(rid_str);
    size_t fname_str_len = ngx_strlen(fname_str);
    ...
    *error_msg = NULL;
    ...
    if (regex == NULL) {
        if(rule->re_precomp == 0)   {
            *error_msg = "Internal Error: regex data is null.";
            ...
            re_pattern->value = ngx_pstrmemdup(msr->mp, rule->re_str, strlen(rule->re_str));
            ...
            if (regex == NULL) {
                *error_msg = ngx_psprintf(rule->ruleset->mp, 
                        ngx_strlen("Error compiling pattern (offset ): ")+ngx_strlen(errptr)+NGX_INT64_LEN+2,
                        "Error compiling pattern (offset %d): %s",erroffset, errptr);
                return 0;
            }
            ...
            if (msr->txcfg->debuglog_level >= 4) {
                ...
                if ((rc != 0) || (jit != 1)) {
                    *error_msg = ngx_psprintf(rule->ruleset->mp,
                            NGX_INT64_LEN*3+ngx_strlen("Rule  [id \"\"][file \"\"][line \"\"] - Execution error - Does not support JIT ()")+rid_str_len+fname_str_len+10,
                            
                            "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                            "Execution error - "
                            "Does not support JIT (%d)",
                            rule,rid_str,fname_str,rule->line_num,rc);
    ...
    if ((rc == PCRE_ERROR_MATCHLIMIT) || (rc == PCRE_ERROR_RECURSIONLIMIT)) {
        ...
        *error_msg = ngx_psprintf(msr->mp, 
                NGX_INT64_LEN*3+ngx_strlen("Rule  [id \"\"][file \"\"][line \"\"] - Execution error - PCRE limits exceeded (): ")+rid_str_len+fname_str_len+ngx_strlen(my_error_msg)+10,
                "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                "Execution error - "
                "PCRE limits exceeded (%d): %s",
                rule,rid_str,fname_str,rule->line_num,rc, my_error_msg);
        ...
        *error_msg = ngx_psprintf(msr->mp,NGX_INT64_LEN+ngx_strlen("Regex execution failed (): ")+ngx_strlen(my_error_msg)+2, 
    ...
    if (rc != PCRE_ERROR_NOMATCH) { /* Match. */
        ...
        char *pattern = ngx_pstrndup(msr->mp, log_escape(msr->mp, regex->pattern ? regex->pattern : "<Unknown Match>"));
        char *hmac = NULL, *valid = NULL;
        char *hash_link = NULL, *nlink = NULL;
        
        size_t pattern_len = ngx_strlen(pattern);

        if (pattern_len > 252) {
            *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Request URI matched \" ...\" at .")+252+ngx_strlen(var->name)+2, 
                    "Request URI matched \"%*s ...\" at %s.",252,pattern, var->name);
        } else {
            *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Request URI matched \"\" at .")+pattern_len+ngx_strlen(var->name)+2, 
                    "Request URI matched \"%s\" at %s.",
                    pattern, var->name);
        }

        valid = ngx_strstr(target, msr->txcfg->crypto_param_name);
        ...
        if(valid == NULL)   {
            ...
            if (strlen(pattern) > 252) {
                *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Request URI matched \" ...\" at . No Hash parameter")+252+ngx_strlen(var->name)+2,
                        "Request URI matched \"%*s ...\" at %s. No Hash parameter",
                        252,pattern, var->name);
            } else {
                *error_msg = ngx_psprintf(msr->mp, 
                        ngx_strlen("Request URI matched \"\" at . No Hash parameter")+pattern_len+ngx_strlen(var->name)+2,
                        "Request URI matched \"%s\" at %s. No Hash parameter",
                        pattern, var->name);
            }
            return 1;
            ...
            if(ngx_strlen(valid) < ngx_strlen(msr->txcfg->crypto_param_name)+1)
                return 1;

            hmac  = valid+ngx_strlen(msr->txcfg->crypto_param_name)+1;

            nlink = ngx_pstrmemdup(msr->mp, target, ngx_strlen(target) - ngx_strlen(valid) - 1);

            msr_log(msr, 9, "Validating URI %s size %zu",nlink,ngx_strlen(nlink));

            hash_link = do_hash_link(msr, (char *)nlink, HASH_ONLY);

            if(ngx_strcmp(hmac, hash_link) != 0)    {

                if (ngx_strlen(pattern) > 252) {
                    *error_msg = ngx_psprintf(msr->mp,
                            ngx_strlen("Request URI matched \" ...\" at . Hash parameter hash value = [] Requested URI hash value = []")+252+ngx_strlen(var->name)+ngx_strlen(hmac)+ngx_strlen(hash_link)+2, 
                            "Request URI matched \"%.252s ...\" at %s. Hash parameter hash value = [%s] Requested URI hash value = [%s]",
                            pattern, var->name, hmac, hash_link);
                } else {
                    *error_msg = ngx_psprintf(msr->mp, 
                            ngx_strlen("Request URI matched \"\" at . Hash parameter hash value = [] Requested URI hash value = []")+pattern_len+ngx_strlen(var->name)+ngx_strlen(hmac)+ngx_strlen(hash_link)+2,
                            "Request URI matched \"%s\" at %s. Hash parameter hash value = [%s] Requested URI hash value = [%s]",
                            pattern, var->name, hmac, hash_link);
                }
                return 1;
            }
...
static int msre_op_rx_param_init(msre_rule *rule, char **error_msg) {
    ...
    const char *pattern = rule->op_param;
       ...
       #ifdef WITH_PCRE_JIT
    int rc, jit;
    const char* rid_str = ((rule->actionset != NULL)&&((rule->actionset->id != NULL)&&(rule->actionset->id != NGX_CONF_UNSET_PTR))) ? rule->actionset->id : "-";
    const char* fname_str = rule->filename != NULL ? rule->filename : "-";
    size_t rid_str_len = ngx_strlen(rid_str);
    size_t fname_str_len = ngx_strlen(fname_str);
       #endif
    #endif
    ...
    *error_msg = NULL;
    ...
    if(ngx_strstr(pattern,"%{") == NULL)    {
        regex = msc_pregcomp_ex(rule->ruleset->mp, pattern, PCRE_DOTALL | PCRE_DOLLAR_ENDONLY, &errptr, &erroffset, msc_pcre_match_limit, msc_pcre_match_limit_recursion);
        if (regex == NULL) {
            *error_msg = ngx_psprintf(rule->ruleset->mp,ngx_strlen("Error compiling pattern (offset ): ")+NGX_INT64_LEN+ngx_strlen(errptr)+2, 
                    "Error compiling pattern (offset %d): %s",
                    erroffset, errptr);
            return 0;
        }
        ...
        if ((rc != 0) || (jit != 1)) {
            *error_msg = ngx_psprintf(rule->ruleset->mp,
                    ngx_strlen("Rule  [id \"\"][file \"\"][line \"\"] - Execution error - Does not support JIT ()")+NGX_INT64_LEN*3+rid_str_len+fname_str_len+10,
                    "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                    "Execution error - "
                    "Does not support JIT (%d)",
                    rule,rid_str,fname_str,rule->line_num,rc);
        ...
        rule->re_precomp = 1;
        rule->re_str = ngx_pstrndup(rule->ruleset->mp, pattern);
        rule->op_param_data = NULL;
    }
...
static int msre_op_rx_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (regex == NULL) {
        if(rule->re_precomp == 0)   {
            *error_msg = "Internal Error: regex data is null.";
            ...
            regex = msc_pregcomp_ex(rule->ruleset->mp, pattern, PCRE_DOTALL | PCRE_DOLLAR_ENDONLY, &errptr, &erroffset, msc_pcre_match_limit, msc_pcre_match_limit_recursion);
            if (regex == NULL) {
                *error_msg = ngx_psprintf(rule->ruleset->mp,ngx_strlen("Error compiling pattern (offset ): ")+ngx_strlen(errptr)+NGX_INT64_LEN+2, 
                        "Error compiling pattern (offset %d): %s",
                        erroffset, errptr);
                return 0;
            }
            ...
            if (msr->txcfg->debuglog_level >= 4) {
                rc = msc_fullinfo(regex, PCRE_INFO_JIT, &jit);
                if ((rc != 0) || (jit != 1)) {
                    *error_msg = ngx_psprintf(rule->ruleset->mp,ngx_strlen("Rule  [id \"\"][file \"\"][line \"\"] - Execution error - Does not support JIT ()")+NGX_INT64_LEN*3+rid_str_len+fname_str_len+10,
                            "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                            "Execution error - "
                            "Does not support JIT (%d)",
                            rule,rid_str,fname_str,rule->line_num,rc);
    ...
    capture = ngx_table_get(rule->actionset->actions, "capture") ? 1 : 0;
    matched_bytes = ngx_table_get(rule->actionset->actions, "sanitizeMatchedBytes") ? 1 : 0;
    if(!matched_bytes)
        matched_bytes = ngx_table_get(rule->actionset->actions, "sanitiseMatchedBytes") ? 1 : 0;

    matched = ngx_table_get(rule->actionset->actions, "sanitizeMatched") ? 1 : 0;
    if(!matched)
        matched = ngx_table_get(rule->actionset->actions, "sanitiseMatched") ? 1 : 0;
    ...
    if ((rc == PCRE_ERROR_MATCHLIMIT) || (rc == PCRE_ERROR_RECURSIONLIMIT)) {
        ...
        *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Rule  [id \"\"][file \"\"][line \"\"] - Execution error - PCRE limits exceeded (): ")+ngx_strlen(my_error_msg)+NGX_INT64_LEN*3+rid_str_len+fname_str_len+10,
                "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                "Execution error - "
                "PCRE limits exceeded (%d): %s",
                rule,rid_str,fname_str_len,rule->line_num,rc, my_error_msg);
        ...
        *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Regex execution failed (): ")+ngx_strlen(my_error_msg)+NGX_INT64_LEN+2, 
                "Regex execution failed (%d): %s",
                rc, my_error_msg);
        return -1;
    }
    ...
    if (capture && rc > 0) {
        ...
        ngx_table_unset(msr->tx_vars, "0");
        ngx_table_unset(msr->tx_vars, "1");
        ngx_table_unset(msr->tx_vars, "2");
        ngx_table_unset(msr->tx_vars, "3");
        ngx_table_unset(msr->tx_vars, "4");
        ngx_table_unset(msr->tx_vars, "5");
        ngx_table_unset(msr->tx_vars, "6");
        ngx_table_unset(msr->tx_vars, "7");
        ngx_table_unset(msr->tx_vars, "8");
        ngx_table_unset(msr->tx_vars, "9");

        /* Use the available captures. */
        for(i = 0; i < rc; i++) {
            msc_string *s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
            if (s == NULL) return -1;
            s->name = ngx_psprintf(msr->mp, NGX_INT64_LEN+2,"%d", i);
            if (s->name == NULL) return -1;
            s->name_len = ngx_strlen(s->name);
            s->value = ngx_pstrmemdup(msr->mp,
                    target + ovector[2 * i], ovector[2 * i + 1] - ovector[2 * i]);
            if (s->value == NULL) return -1;

            s->value_len = (ovector[2 * i + 1] - ovector[2 * i]);

            ngx_table_addn(msr->tx_vars, s->name, (void *)s);

            if(((matched == 1) || (matched_bytes == 1)) && (var != NULL) && (var->name != NULL))    {
                qspos = ngx_pstrndup(msr->mp,var->name);
                parm = ngx_strstr(qspos, ":");
                if (parm != NULL)   {
                    parm++;
                    mparm = (msc_parm*)ngx_palloc(msr->mp, sizeof(msc_parm));
                    if (mparm == NULL)
                        continue;

                    mparm->value = ngx_pstrmemdup(msr->mp,s->value,s->value_len);
                    mparm->pad_1 = rule->actionset->arg_min;
                    mparm->pad_2 = rule->actionset->arg_max;
                    ngx_table_addn(msr->pattern_to_sanitize, parm, (void *)mparm);
                } else  {
                    mparm = (msc_parm*)ngx_palloc(msr->mp, sizeof(msc_parm));
                    if (mparm == NULL)
                        continue;
                    ...
                    mparm->value = ngx_pstrmemdup(msr->mp,s->value,s->value_len);
                    ngx_table_addn(msr->pattern_to_sanitize, qspos, (void *)mparm);
                }
            }
    ...
    if (rc != PCRE_ERROR_NOMATCH) { /* Match. */
        ...
        if (pattern_len > 252) {
            *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Pattern match \" ...\" at .")+ngx_strlen(var->name)+252+2, 
                    "Pattern match \"%*s ...\" at %s.",
                    252,pattern, var->name);
        } else {
            *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Pattern match \"\" at .")+ngx_strlen(var->name)+pattern_len+2, 
                    "Pattern match \"%s\" at %s.",
                    pattern, var->name);
        }
...
static int msre_op_pm_param_init(msre_rule *rule, char **error_msg) {
    ...
    if ((rule->op_param == NULL)||(strlen(rule->op_param) == 0)) {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Missing parameter for operator 'pm'.");
        return 0; /* ERROR */
    }

    op_len = ngx_strlen(rule->op_param);

    p = acmp_create(0, rule->ruleset->mp);
    if (p == NULL) return 0;

    phrase = ngx_pstrndup(rule->ruleset->mp, parse_pm_content(rule->op_param, op_len, rule, error_msg));

    if(phrase == NULL)
        phrase = ngx_pstrndup(rule->ruleset->mp, rule->op_param);
...
static int msre_op_pmFromFile_param_init(msre_rule *rule, char **error_msg) {
    ...
    if ((rule->op_param == NULL)||(ngx_strlen(rule->op_param) == 0)) {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Missing parameter for operator 'pmFromFile'.");
        return 0; /* ERROR */
    }
    ...
    rulefile_path = ngx_pstrmemdup(rule->ruleset->mp, rule->filename, ngx_strlen(rule->filename) - ngx_strlen(ngx_filepath_name_get(rule->filename)));
    ...
    for (;;) {
        ...
        /* Trim whitespace */
        while((ngx_isspace(*fn) != 0) && (*fn != '\0')) fn++;
        if (*fn == '\0') break;
        next = fn;
        while((ngx_isspace(*next) == 0) && (*next != '\0')) next++;
        while((ngx_isspace(*next) != 0) && (*next != '\0')) *(next++) = '\0';

        /* Add path of the rule filename for a relative phrase filename */
        filepath = fn;
        if (ngx_filepath_root(&rootpath, &filepath, NGX_FILEPATH_TRUENAME, rule->ruleset->mp) != NGX_OK) {
            /* We are not an absolute path.  It could mean an error, but
             * let that pass through to the open call for a better error */
            ngx_filepath_merge(&fn, rulefile_path, fn, NGX_FILEPATH_TRUENAME, rule->ruleset->mp);
        }

        /* Open file and read */
        file  = ngx_open_file_read(rule->ruleset->mp,fn);
        if (file == NULL) {
            *error_msg = ngx_psprintf(rule->ruleset->mp,ngx_strlen("Could not open phrase file \"\": ")+ngx_strlen(fn)+2,
        ...
        for(;;) {
            line++;
            rc = ngx_file_gets(buf, HUGE_STRING_LEN, file);
            if (rc == NGX_EOF) break;
            if (rc != NGX_OK) {
                *error_msg = ngx_psprintf(rule->ruleset->mp,ngx_strlen("Could not read \"\" line ")+ngx_strlen(fn)+NGX_INT64_LEN+2, 
                        "Could not read \"%s\" line %d", fn, line);
                return 0;
            }

            op_len = ngx_strlen(buf);
            processed = ngx_pstrndup(rule->ruleset->mp, parse_pm_content(buf, op_len, rule, error_msg));
            ...
            while ((ngx_isspace(*start) != 0) && (*start != '\0')) start++;
            if(processed != NULL)
                end = processed + ngx_strlen(processed);
            else
                end = buf + ngx_strlen(buf);
            if (end > start) end--;
            while ((end > start) && (ngx_isspace(*end) != 0)) end--;
            if (end > start) {
                *(++end) = '\0';
            }
    ...
    if (file != NULL) ngx_close_file(file->fd);
    acmp_prepare(p);
    rule->op_param_data = p;
    return 1;
}
...
static int msre_op_pm_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    if (rc) {
        ...
        /* This message will be logged. */
        if (ngx_strlen(match_escaped) > 252) {
            *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Matched phrase \" ...\" at .")+ngx_strlen(var->name)+252+2,
                    "Matched phrase \"%*s ...\" at %s.",
                    252,match_escaped, var->name);
        } else {
            *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Matched phrase \"\" at .")+ngx_strlen(match_escaped)+ngx_strlen(var->name)+2,
                    "Matched phrase \"%s\" at %s.",
        ...
        if (capture) {
            ...
            s->name = "0";
            s->name_len = ngx_strlen(s->name);
            s->value = ngx_pstrndup(msr->mp, match);
            if (s->value == NULL) return -1;
            s->value_len = ngx_strlen(s->value);
            ngx_table_setn(msr->tx_vars, s->name, (void *)s);
            ...
            for(i = rc; i <= 9; i++) {
                char buf[2];
                ngx_snprintf((u_char*)buf, sizeof(buf), "%d", i);
                ngx_table_unset(msr->tx_vars, buf);
            }
        }
 ...
 */
static const char *gsb_replace_tpath(ngx_pool_t *pool, const char *domain, int len)    {

    char *pos = NULL, *data = NULL;
    char *url = NULL;
    int match = 0;

    url = (char*)ngx_palloc(pool, len + 1);
    data = (char*)ngx_palloc(pool, len + 1);
 ...
 */
static const char *gsb_reduce_char(ngx_pool_t *pool, const char *domain) {

    char *ptr = ngx_pstrndup(pool, domain);
    char *data = NULL;
    char *reduced = NULL;
    int skip = 0;
    ...
    data = (char*)ngx_pcalloc(pool, strlen(ptr));
...
static int verify_gsb(gsb_db *gsb, modsec_rec *msr, const char *match, unsigned int match_length) {
    ngx_md5_t ctx;
    ngx_int_t rc;
    char digest[NGX_MD5_DIGESTSIZE];
    ...
    if ((rc = ngx_md5_update(&ctx, (const void *)match, match_length)) != NGX_OK)
        return -1;

    ngx_md5_final((u_char*)digest, &ctx);

    hash = ngx_pstrndup(msr->mp,bytes2hex(msr->mp, (unsigned char*)digest, 16));

    if ((hash != NULL) && (gsb->gsb_table != NULL))   {
        search = ngx_hash2_get(gsb->gsb_table, hash, NGX_HASH_KEY_STRING);
...
static int msre_op_gsbLookup_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    data = (char*)ngx_pcalloc(rule->ruleset->mp, var->value_len+1);
    ...
    while (offset < size && (rv = msc_regexec_ex(regex, data, size, offset, PCRE_NOTEMPTY, ovector, 30, &my_error_msg)) >= 0)
    {
        for(i = 0; i < rv; ++i)
        {
            match = ngx_psprintf(rule->ruleset->mp, ovector[2*i+1] - ovector[2*i]+10,
                    "%*s", ovector[2*i+1] - ovector[2*i], data + ovector[2*i]);
            ...
            if((strstr(match,"http") == NULL) && (match_length > 0) && (strchr(match,'.')))    {
                ...
                if(ret > 0) {
                    ...
                    if (! *error_msg) {
                        char *nname = log_escape_nq(msr->mp, match);
                        size_t nname_len = ngx_strlen(nname);

                        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Gsb lookup for \"\" succeeded.")+nname_len+2,
                                "Gsb lookup for \"%s\" succeeded.",
                                nname);
                    }

                    str = ngx_pstrndup(rule->ruleset->mp,match);

                    base = ngx_strtok(str,"/",&savedptr);
                    if(base != NULL)
                ...
                if ((match[match_length -1] != '/') && (strchr(match,'?') == NULL))    {
                    ...
                    canon = ngx_pstrcat(rule->ruleset->mp, match,"/",NULL);
                    if (canon != NULL)  {

                        canon_length = ngx_strlen(canon);
                        ...
                        if(ret > 0) {
                            ...
                            if (! *error_msg) {
                                *error_msg = ngx_pstrcat(msr->mp, 
                                        "Gsb lookup for \"",log_escape_nq(msr->mp, canon),"\" succeeded.",NULL);
                            }
                ...
                domain = ngx_pstrndup(rule->ruleset->mp, match);

                domain_len = ngx_strlen(domain);
                ...
                if(*domain != '/')  {
                    ...
                    dot = strchr(domain,'.');
                    if(dot != NULL) {
                        canon = ngx_pstrndup(rule->ruleset->mp, domain);
                        ...
                        if(ret > 0) {
                            ...
                            if (! *error_msg) {
                                *error_msg = ngx_pstrcat(msr->mp, "Gsb lookup for \" ",
                                        log_escape_nq(msr->mp, canon),"\" succeeded.",NULL);
                            }
                        ...
                        base = ngx_strtok(canon,"?",&savedptr);

                        if(base != NULL)   {
                            ret = verify_gsb(gsb, msr, base, ngx_strlen(base));

                            if(ret > 0) {
                                set_match_to_tx(msr, capture, base, 0);
                                if (! *error_msg) {
                                    *error_msg = ngx_pstrcat(msr->mp,
                                            "Gsb lookup for \" ",log_escape_nq(msr->mp, base),"\" succeeded.",NULL);
                                }
                        ...
                        url = ngx_palloc(rule->ruleset->mp, strlen(canon));
                        count_slash = 0;

                        while(*canon != '\0') {
                            switch (*canon)   {
                                case '/':
                                    ptr = ngx_pstrcat(rule->ruleset->mp,url,"/",NULL);
                                    ret = verify_gsb(gsb, msr, ptr, strlen(ptr));
                                    if(ret > 0) {
                                        set_match_to_tx(msr, capture, ptr, 0);
                                        if (! *error_msg) {
                                            *error_msg = ngx_pstrcat(msr->mp, 
                                                    "Gsb lookup for \" ", log_escape_nq(msr->mp, ptr)," \" succeeded.",NULL);
                                        }
                        ...
                        case '.':
                            ...
                            if(*domain != '/')  {
                                ...
                                if(dot != NULL) {
                                    ...
                                    ret = verify_gsb(gsb, msr, canon, ngx_strlen(canon));
                                        ...
                                        switch (*canon)   {
                                            case '/':
                                                ...
                                                if(ret > 0) {
                                                    set_match_to_tx(msr, capture, ptr, 0);
                                                    if (! *error_msg) {
                                                        *error_msg = ngx_psprintf(msr->mp,64, 
                                                            "Gsb lookup for \"",log_escape_nq(msr->mp, ptr),"\" succeeded.",NULL);
                                                    }
                                                    str = ngx_pstrndup(rule->ruleset->mp,match);
...
static int msre_op_within_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (target_length == 0) {
        /* Match. */
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("String match within \"\" at .")+ngx_strlen(var->name)+2,
                "String match within \"\" at %s.",
                var->name);
        return 1;
    }
    ...
    for (i = 0; i <= i_max; i++) {
        if (match[i] == target[0]) {
            if (memcmp((target + 1), (match + i + 1), (target_length - 1)) == 0) {
                /* match. */
                char *nname = log_escape_ex(msr->mp, match, match_length);
                size_t nname_len = ngx_strlen(nname);

                *error_msg = ngx_psprintf(msr->mp,ngx_strlen("String match within \"\" at .")+nname_len+ngx_strlen(var->name)+2, 
                        "String match within \"%s\" at %s.",
                        nname,
                        var->name);
                return 1;
            }
...
static int msre_op_contains_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (match_length == 0) {
        /* Match. */
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("String match \"\" at .")+ngx_strlen(var->name)+2,"String match \"\" at %s.", var->name);
        return 1;
    }
...
static int msre_op_detectSQLi_execute(modsec_rec *msr, msre_rule *rule, msre_var *var,
    ...
    if (issqli) {
        ...
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("detected SQLi using libinjection with fingerprint ''")+ngx_strlen(fingerprint)+2,
                "detected SQLi using libinjection with fingerprint '%s'", fingerprint);
...
static int msre_op_detectXSS_execute(modsec_rec *msr, msre_rule *rule, msre_var *var,
    ...
    if (is_xss) {
        *error_msg = ngx_pstrndup(msr->mp, "detected XSS using libinjection.");
...
static int msre_op_containsWord_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (match_length == 0) {
        /* Match. */
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("String match \"\" at .")+ngx_strlen(var->name)+2,
                "String match \"\" at %s.", var->name);
        return 1;
    }
    ...
    if (rc == 1) {
        ...
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("String match \"\" at .")+ngx_strlen(var->name)+nname_len+2,
                "String match \"%s\" at %s.",nname,var->name);
        return 1;
    }
...
static int msre_op_strmatch_param_init(msre_rule *rule, char **error_msg) {
    const ngx_strmatch_pattern *compiled_pattern;
    char *processed = NULL;
    const char *pattern = rule->op_param;
    unsigned short int op_len;
    ...
    *error_msg = NULL;
    ...
    /* Compile pattern */
    compiled_pattern = ngx_strmatch_precompile(rule->ruleset->mp, processed, 1);
    if (compiled_pattern == NULL) {
        *error_msg = ngx_pstrcat(rule->ruleset->mp, "Error compiling pattern: ", pattern,NULL);
        return 0;
    }
...
static int msre_op_strmatch_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ngx_strmatch_pattern *compiled_pattern = (ngx_strmatch_pattern *)rule->op_param_data;
    const char *target;
    unsigned int target_length;
    const char *rc;
    char *nname;
    ...
    *error_msg = NULL;
    ...
    rc = ngx_strmatch(compiled_pattern, target, target_length);
    if (rc == NULL) {
        /* No match. */
        return 0;
    }
    
    nname = log_escape(msr->mp, rule->op_param);
    nname_len = ngx_strlen(nname);

    *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Pattern match \"\" at .")+ngx_strlen(var->name)+nname_len+2,
            "Pattern match \"%s\" at %s.",nname, var->name);
        ...
        char **error_msg)
{
    ...
    if (dtd == NULL) {
        *error_msg = ngx_pstrcat(msr->mp, "XML: Failed to load DTD: ", rule->op_param,NULL);
        return -1;
    }
        ...
        char **error_msg)
{
    ...
    if (parserCtx == NULL) {
        *error_msg = ngx_pstrcat(msr->mp, "XML: Failed to load Schema from file: ",rule->op_param,NULL);
        return -1;
    }
    ...
    if (schema == NULL) {
        *error_msg = ngx_pstrcat(msr->mp, "XML: Failed to load Schema: ", rule->op_param,NULL);
...
static int msre_op_verifyCC_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    size_t rid_str_len = ngx_strlen(rid_str);
    const char *fname_str = rule->filename != NULL ? rule->filename : "-";
    size_t fname_strlen = ngx_strlen(fname_str);
       #endif
    #endif
    ...
    *error_msg = NULL;
    ...
    for (offset = 0; ((unsigned int)offset < target_length) && (is_cc == 0); offset++) {
        ...
        if (rc < -1) {
            *error_msg = ngx_pstrcat(msr->mp, "CC# regex execution failed: ", my_error_msg,NULL);
            return -1;
        }
        ...
        if (rc > 0) {
            ...
            if (ngx_table_get(rule->actionset->actions, "capture")) {
                for(; i < rc; i++) {
                    msc_string *s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
                    if (s == NULL) return -1;
                    s->name = ngx_psprintf(msr->mp, NGX_INT64_LEN,"%d", i);
                    if (s->name == NULL) return -1;
                    s->name_len = ngx_strlen(s->name);
                    s->value = ngx_pstrmemdup(msr->mp, match, length);
                    if (s->value == NULL) return -1;
                    s->value_len = length;
                    ...
                    if((matched_bytes == 1) && (var != NULL) && (var->name != NULL))    {
                        ...
                        if (parm != NULL)   {
                            parm++;
                            mparm = ngx_palloc(msr->mp, sizeof(msc_parm));
                            if (mparm == NULL)
                                continue;
    ...
    if (is_cc) {
        ...
        /* This message will be logged. */
        *error_msg = ngx_psprintf(msr->mp, 
                ngx_strlen("CC# match \"\" at . [offset \"\"]")+ngx_strlen(regex->pattern)+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                "CC# match \"%s\" at %s. [offset \"%d\"]",
...
static int cpf_verify(const char *cpfnumber, int len) {
    ...
    while((*cpfnumber != '\0') && ( var_len > 0))  {
        ...
        if(*cpfnumber != '-' || *cpfnumber != '.') {
            if(i < cpf_len && ngx_isdigit(*cpfnumber))  {
                s_cpf[i] = *cpfnumber;
...
static int msre_op_verifyCPF_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (msr->txcfg->debuglog_level >= 4) {
        ...
        if ((rc != 0) || (jit != 1)) {
            *error_msg = ngx_psprintf(rule->ruleset->mp,
                    ngx_strlen("Rule %pp [id \"\"][file \"\"][line \"\"] - Execution error - Does not support JIT ()")+NGX_INT64_LEN*3+rid_str_len+fname_str_len+10,
                    "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                    "Execution error - "
                    "Does not support JIT (%d)",
                    rule,rid_str,fname_str,rule->line_num,rc);
    ...
    for (offset = 0; ((unsigned int)offset < target_length) && (is_cpf == 0); offset++) {
        ...
        if (rc < -1) {
            *error_msg = ngx_pstrcat(msr->mp, "CPF# regex execution failed: ", my_error_msg,NULL);
            return -1;
        }
    ...
    if (is_cpf) {
        ...
        /* This message will be logged. */
        *error_msg = ngx_psprintf(msr->mp, 
                ngx_strlen("CPF# match \"\" at . [offset \"\"]")+ngx_strlen(regex->pattern)+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                "CPF# match \"%s\" at %s. [offset \"%d\"]",
            regex->pattern, var->name, offset);
...
static int ssn_verify(modsec_rec *msr, const char *ssnumber, int len) {
    ...
    str_area = ngx_psprintf(msr->mp,NGX_INT64_LEN*3+2,"%d%d%d",num[0],num[1],num[2]);
    str_grp = ngx_psprintf(msr->mp,NGX_INT64_LEN*2+2,"%d%d",num[3],num[4]);
    str_serial = ngx_psprintf(msr->mp,NGX_INT64_LEN*4+2,"%d%d%d%d",num[5],num[6],num[7],num[8]);
...
static int msre_op_verifySSN_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (msr->txcfg->debuglog_level >= 4) {
        ...
        if ((rc != 0) || (jit != 1)) {
            *error_msg = ngx_psprintf(rule->ruleset->mp,
                    ngx_strlen("Rule  [id \"\"][file \"\"][line \"\"] - Execution error - Does not support JIT ()")+rid_str_len+fname_str_len+NGX_INT64_LEN*3+10,
                    "Rule %pp [id \"%s\"][file \"%s\"][line \"%d\"] - "
                    "Execution error - "
                    "Does not support JIT (%d)",
                    rule,rid_str,fname_str,rule->line_num,rc);
    ...
    for (offset = 0; ((unsigned int)offset < target_length) && (is_ssn == 0); offset++) {
        ...
        if (rc < -1) {
            *error_msg = ngx_pstrcat(msr->mp, "SSN# regex execution failed: ", my_error_msg,NULL);
            return -1;
        }
    ...
    if (is_ssn) {
        ...
        /* This message will be logged. */
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("SSN# match \"\" at . [offset \"\"]")+ngx_strlen(regex->pattern)+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                "SSN# match \"%s\" at %s. [offset \"%d\"]",
            regex->pattern, var->name, offset);
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (rc <= 0) {
        if (! *error_msg) {
            *error_msg = ngx_pstrcat(msr->mp, "Geo lookup for \"",log_escape_nq(msr->mp, geo_host),"\" failed at ",var->name,".",NULL); 
        }
        ngx_table_clear(msr->geo_vars);
        return rc;
    }
    if (! *error_msg) {
        *error_msg = ngx_pstrcat(msr->mp, "Geo lookup for \"",log_escape_nq(msr->mp, geo_host),"\" succeeded at ",var->name,".",NULL);
    }
    ...
    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "COUNTRY_CODE");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_pstrndup(msr->mp, rec.country_code ? rec.country_code : "");
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "COUNTRY_CODE3");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_pstrndup(msr->mp, rec.country_code3 ? rec.country_code3 : "");
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "COUNTRY_NAME");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_pstrndup(msr->mp, rec.country_name ? rec.country_name : "");
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "COUNTRY_CONTINENT");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_pstrndup(msr->mp, rec.country_continent ? rec.country_continent : "");
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "REGION");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_pstrndup(msr->mp, rec.region ? rec.region : "");
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "CITY");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_pstrndup(msr->mp, rec.city ? rec.city : "");
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "POSTAL_CODE");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_pstrndup(msr->mp, rec.postal_code ? rec.postal_code : "");
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "LATITUDE");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_psprintf(msr->mp, NGX_INT64_LEN+2,"%f", rec.latitude);
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "LONGITUDE");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_psprintf(msr->mp, NGX_INT64_LEN+2,"%f", rec.longitude);
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "DMA_CODE");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_psprintf(msr->mp, NGX_INT64_LEN+2,"%d", rec.dma_code);
    s->value_len = ngx_strlen(s->value);
    ngx_table_setn(msr->geo_vars, s->name, (void *)s);

    s = (msc_string *)ngx_pcalloc(msr->mp, sizeof(msc_string));
    s->name = ngx_pstrndup(msr->mp, "AREA_CODE");
    s->name_len = ngx_strlen(s->name);
    s->value = ngx_psprintf(msr->mp, NGX_INT64_LEN+2,"%d", rec.area_code);
...
static int msre_op_rbl_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    target = ngx_pstrmemdup(msr->mp, var->value, var->value_len);
        ...
        if(strstr(rule->op_param,"httpbl.org"))  {
            if (msr->txcfg->httpBlkey == NULL) {
                ...
                *error_msg = "RBL httpBl called but no key defined: set SecHttpBlKey";
                ...
                name_to_check = ngx_psprintf(msr->mp,
                        ngx_strlen(msr->txcfg->httpBlkey)+ngx_strlen(rule->op_param)+NGX_INT64_LEN*4+2,
                        "%s.%d.%d.%d.%d.%s", msr->txcfg->httpBlkey, h3, h2, h1, h0, rule->op_param);
            }
        } else {
            /* regular IPv4 RBLs */
            name_to_check = ngx_psprintf(msr->mp, 
                    NGX_INT64_LEN*4+ngx_strlen(rule->op_param)+2,
                    "%d.%d.%d.%d.%s", h3, h2, h1, h0, rule->op_param);
        }
    } else {
        /* Assume the input is a domain name. */
        name_to_check = ngx_psprintf(msr->mp, "%s.%s", target, rule->op_param);
    }

    if (name_to_check == NULL) return -1;

    rc = ngx_sockaddr_info_get(&sa, name_to_check,
        NGX_UNSPEC/*msr->r->connection->remote_addr->family*/, 0, 0, msr->mp);
    if (rc == NGX_OK) {
        ...
        if(strstr(rule->op_param,"uribl.com"))  {
            ...
            switch(high8bits)   {
                case 2:
                    *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (BLACK).",
                            log_escape_nq(msr->mp, name_to_check), var->name);
                    break;
                case 4:
                    *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (GREY).",
                            log_escape_nq(msr->mp, name_to_check), var->name);
                    break;
                case 8:
                    *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (RED).",
                            log_escape_nq(msr->mp, name_to_check), var->name);
                    break;
                case 14:
                    *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (BLACK,GREY,RED).",
                            log_escape_nq(msr->mp, name_to_check), var->name);
                    break;
                case 255:
                    *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (DNS IS BLOCKED).",
                            log_escape_nq(msr->mp, name_to_check), var->name);
                    break;
                default:
                    *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (WHITE).",
                            log_escape_nq(msr->mp, name_to_check), var->name);
                    break;
            }
            ...
            if(strstr(rule->op_param,"spamhaus.org"))  {
                ...
                switch(high8bits)   {
                    case 2:
                    case 3:
                        *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (Static UBE sources).",
                                log_escape_nq(msr->mp, name_to_check), var->name);
                        break;
                    case 4:
                    case 5:
                    case 6:
                    case 7:
                        *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (Illegal 3rd party exploits).",
                                log_escape_nq(msr->mp, name_to_check), var->name);
                        break;
                    case 10:
                    case 11:
                        *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s (Delivering unauthenticated SMTP email).",
                                log_escape_nq(msr->mp, name_to_check), var->name);
                        break;
                    default:
                        *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s.",
                                log_escape_nq(msr->mp, name_to_check), var->name);
                        break;
                }
            ...
            set_match_to_tx(msr, capture, *error_msg, 0);
                ...
                if(strstr(rule->op_param,"httpbl.org"))  {
                   ...
                   if (sscanf(respBl, "%d.%d.%d.%d", &first, &days, &score, &type) != 4) {
                       *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s failed: bad response", log_escape_nq(msr->mp, name_to_check));
                   } else {
                       if (first != 127) {
                           *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s failed: bad response", log_escape_nq(msr->mp, name_to_check));
                       }
                           ...
                           *error_msg = ngx_psprintf(msr->r->pool, "RBL lookup of %s succeeded at %s. %s: %d days since last activity, threat score %d",
                               log_escape_nq(msr->mp, name_to_check), var->name,
                               ptype, days, score);
                       }
                   }
...
static int msre_op_fuzzy_hash_init(msre_rule *rule, char **error_msg)
{
    ...
    param_data = ngx_palloc(rule->ruleset->mp,
        sizeof(struct fuzzy_hash_param_data));

    data = ngx_pstrndup(rule->ruleset->mp, rule->op_param);
    threshold_str = data;
#endif
...
#ifdef WITH_SSDEEP
    /* Sanity check */
    param_len = ngx_strlen(threshold_str);
    ...
    *threshold_str = '\0';
    ...
    if (!fopen(file, "r"))
    {
        *error_msg = ngx_pstrcat(rule->ruleset->mp, "Not able to open file:" \
            " ", file,NULL);
        return -1;
    }
...
#ifdef WITH_SSDEEP
invalid_parameters:
    *error_msg = ngx_pstrndup(rule->ruleset->mp, "Operator @fuzzyHash " \
    ...
    msre_var *var, char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Problems generating " \
            "fuzzy hash.");
    ...
    if (!fp)
    {
        *error_msg = ngx_pstrcat(rule->ruleset->mp, "Not able to open " \
            "fuzzy hash file: ", param->file,NULL);
    ...
    while (read_line(line, sizeof(line), fp))
    {
        ...
        {
            *error_msg = ngx_psprintf(msr->mp,64,"Fuzzy hash of %s matched " \
    ...
    fclose(fp);
#else
    *error_msg = ngx_pstrndup(rule->ruleset->mp, "ModSecurity was not " \
...
static int msre_op_inspectFile_init(msre_rule *rule, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if ((filename == NULL)||(is_empty_string(filename))) {
        *error_msg = ngx_psprintf(rule->ruleset->mp, "Operator @inspectFile requires parameter.");
        return -1;
    }
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (rule->op_param_data == NULL) {
        ...
        const char *target_file = ngx_pstrmemdup(msr->mp, var->value, var->value_len);
        ...
        if (apache2_exec(msr, approver_script, (const char **)argv, &script_output) <= 0) {
            *error_msg = ngx_psprintf(msr->mp, "Execution of the approver script \"%s\" failed (invocation failed).",
                log_escape(msr->mp, approver_script));
            return -1;
        }

        if (script_output == NULL) {
            *error_msg = ngx_psprintf(msr->mp, "Execution of the approver script \"%s\" failed (no output).",
                log_escape(msr->mp, approver_script));
            return -1;
        }

        if (script_output[0] != '1') {
            *error_msg = ngx_psprintf(msr->mp, "File \"%s\" rejected by the approver script \"%s\": %s",
        ...
        char *target = ngx_pstrmemdup(msr->mp, var->value, var->value_len);
...
static int msre_op_validateByteRange_init(msre_rule *rule, char **error_msg) {
    ...
    *error_msg = NULL;
    ...
    if (rule->op_param == NULL) {
        *error_msg = ngx_pstrndup(rule->ruleset->mp, "Missing parameter for validateByteRange.");
        return -1;
    }

    /* Initialise. */
    data = ngx_pstrndup(rule->ruleset->mp, rule->op_param);
    rule->op_param_data = ngx_pcalloc(rule->ruleset->mp, 32);
    ...
    while(p != NULL) {
        ...
        if (s == NULL) {
            ...
            int x = atoi(p);
            if ((x < 0)||(x > 255)) {
                *error_msg = ngx_psprintf(rule->ruleset->mp, ngx_strlen("Invalid range value: ")+NGX_INT64_LEN+2,
                        "Invalid range value: %d", x);
                return 0;
            }
            ...
            if ((start < 0)||(start > 255)) {
                *error_msg = ngx_psprintf(rule->ruleset->mp, ngx_strlen("Invalid range start value: ")+NGX_INT64_LEN+2,
                        "Invalid range start value: %d",
                    start);
                return 0;
            }
            if ((end < 0)||(end > 255)) {
                *error_msg = ngx_psprintf(rule->ruleset->mp, ngx_strlen("Invalid range end value: ")+NGX_INT64_LEN+2,
                        "Invalid range end value: %d", end);
                return 0;
            }
            if (start > end) {
                *error_msg = ngx_psprintf(rule->ruleset->mp, ngx_strlen("Invalid range: -")+NGX_INT64_LEN*2+2,
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (table == NULL) {
        *error_msg = ngx_pstrndup(msr->mp, "Internal Error: validateByteRange table not "
            "initialised.");
        return -1;
    }
    ...
    *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Found  byte(s) in  outside range: .")+ngx_strlen(var->name)+ngx_strlen(rule->op_param)+NGX_INT64_LEN+2,
            "Found %d byte(s) in %s outside range: %s.",
    ...
    char **error_msg)
{
    ...
    switch(rc) {
        case 1 :
            /* Encoding is valid */
            *error_msg = ngx_pstrcat(msr->mp, "Valid URL Encoding at ", var->name,NULL);
            break;
        case -2 :
            *error_msg = ngx_pstrcat(msr->mp, "Invalid URL Encoding: Non-hexadecimal "
                "digits used at ", var->name,NULL);
            return 1; /* Invalid match. */
            break;
        case -3 :
            *error_msg = ngx_pstrcat(msr->mp, "Invalid URL Encoding: Not enough characters "
                "at the end of input at ", var->name,NULL);
            return 1; /* Invalid match. */
            break;
        case -1 :
        default :
            *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Invalid URL Encoding: Internal Error (rc = ) at ")+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                    "Invalid URL Encoding: Internal Error (rc = %d) at %s", rc, var->name);
            return -1;
            break;
    ...
    for(i = 0; i < var->value_len;) {
        ...
        switch(rc) {
            case UNICODE_ERROR_CHARACTERS_MISSING :
                *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Invalid UTF-8 encoding: not enough bytes in character at . [offset \"\"]")+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                        "Invalid UTF-8 encoding: not enough bytes in character at %s. [offset \"%d\"]", var->name, i);
                return 1;
                break;
            case UNICODE_ERROR_INVALID_ENCODING :
                *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Invalid UTF-8 encoding: invalid byte value in character at . [offset \"\"]")+ngx_strlen(var->name)+NGX_INT64_LEN+2, 
                        "Invalid UTF-8 encoding: invalid byte value in character at %s. [offset \"%d\"]", var->name, i);
                return 1;
                break;
            case UNICODE_ERROR_OVERLONG_CHARACTER :
                *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Invalid UTF-8 encoding: overlong character detected at . [offset \"\"]")+ngx_strlen(var->name)+NGX_INT64_LEN+2, 
                        "Invalid UTF-8 encoding: overlong character detected at %s. [offset \"%d\"]", var->name, i);
                return 1;
                break;
            case UNICODE_ERROR_RESTRICTED_CHARACTER :
                *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Invalid UTF-8 encoding: use of restricted character at . [offset \"\"]")+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                        "Invalid UTF-8 encoding: use of restricted character at %s. [offset \"%d\"]", var->name, i);
                return 1;
                break;
            case UNICODE_ERROR_DECODING_ERROR :
                *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Error validating UTF-8 decoding at . [offset \"\"]")+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                        "Error validating UTF-8 decoding at %s. [offset \"%d\"]", var->name, i);
                return 1;
                break;
        }

        if (rc <= 0) {
            *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Internal error during UTF-8 validation at ")+ngx_strlen(var->name)+2,"Internal error during UTF-8 validation "
                "at %s.", var->name);
            return 1;
        }
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (left != right) {
        ...
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Operator EQ matched  at .")+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                "Operator EQ matched %d at %s.", right, var->name);
        /* Match. */
        return 1;
    }
}
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (left <= right) {
        ...
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Operator GT matched  at .")+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                "Operator GT matched %d at %s.", right, var->name);
        /* Match. */
        return 1;
    }
}
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (left >= right) {
        ...
        *error_msg = ngx_psprintf(msr->mp, ngx_strlen("Operator LT matched  at .")+ngx_strlen(var->name)+NGX_INT64_LEN+2,
                "Operator LT matched %d at %s.", right, var->name);
        /* Match. */
        return 1;
    }
}
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (left < right) {
        ...
        *error_msg = ngx_psprintf(msr->mp,ngx_strlen("Operator GE matched  at .")+ngx_strlen(var->name)+NGX_INT64_LEN+2, 
                "Operator GE matched %d at %s.", right, var->name);
        /* Match. */
        return 1;
    }
}
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (left > right) {
        ...
        *error_msg = ngx_psprintf(msr->mp, 
                ngx_strlen("Operator LE matched  at .")+ngx_strlen(var->name)+NGX_INT64_LEN+2,"Operator LE matched %d at %s.", right, var->name);
