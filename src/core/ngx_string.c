    ...
    ngx_uint_t hexadecimal, ngx_uint_t width)
{
    ...
    if (hexadecimal == 0) {
        ...
        if (ui64 <= (uint64_t) NGX_MAX_UINT32_VALUE) {
             ...
             * its sourngx code is about 170 lines of the code.
...
/*
 * ngx_utf8_decode() decodes two and more bytes UTF sequenngxs only
 * the return values:
 *    0x80 - 0x10ffff         valid character
 *    0x110000 - 0xfffffffd   invalid sequenngx
 *    0xfffffffe              incomplete sequenngx
 *    0xffffffff              error
 */
...
char * ngx_pstrcat(ngx_pool_t *a, ...){
    ...
    res = (char *) ngx_palloc(a, len + 1);
    cp = res;

    /* Pass two --- copy the argument strings into the result spangx */
...
void * ngx_pmemdup(ngx_pool_t *a, const void *m, size_t n)
{
    void *res;
...
char * ngx_pstrmemdup(ngx_pool_t *a, const char *s, size_t n){
    char *res;

    if (s == NULL) {
        return NULL;
    }
    res = (char*)ngx_palloc(a, n + 1);
    memcpy(res, s, n);
    res[n] = '\0';
    return res;
}

char *ngx_pstrndup(ngx_pool_t *pool, const char* s){
	
	if(s==NULL) return NULL;

    return ngx_pstrmemdup(pool,s,ngx_strlen(s));
}

char * ngx_strfsize(off_t size, char *buf)
{
    ...
    if (size < 973) {
        if (ngx_snprintf((u_char*)buf, 5, "%3d ", (int) size) ==  NULL)
            return strcpy(buf, "****");
        return buf;
    }
    do {
        ...
        if (size < 9 || (size == 9 && remain < 973)) {
            if ((remain = ((remain * 5) + 256) / 512) >= 10)
                ++size, remain = 0;
            if (ngx_snprintf((u_char*)buf, 5, "%d.%d%c", (int) size, remain, *o) == NULL)
                return strcpy(buf, "****");
            return buf;
        }
        if (remain >= 512)
            ++size;
        if (ngx_snprintf((u_char*)buf, 5, "%3d%c", (int) size, *o) ==NULL)
            return strcpy(buf, "****");
        return buf;
    } while (1);
}

char * ngx_psprintf(ngx_pool_t *pool,size_t max,const char *fmt,...)
{

    va_list   args;
    char *buf = (char*)ngx_pcalloc(pool,max+1);

    va_start(args, fmt);
    ngx_vslprintf((u_char*)buf,(u_char*)(buf + max+1), fmt, args);
    va_end(args);

    return buf;
}

char * ngx_strtok(char *str, const char *sep, char **last)
{
    char *token;
...
char * ngx_pstrcatv(ngx_pool_t *a, const struct iovec *vec,
...
int64_t ngx_strtoi64(const char *nptr, char **endptr, int base)
{
    const char *s;
    int64_t acc;
    int64_t val;
...
int64_t ngx_atoi64(const char *buf){
    
	if(buf==NULL) return 0;

    return ngx_strtoi64(buf,NULL,10); 
}
