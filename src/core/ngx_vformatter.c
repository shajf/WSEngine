 ...
 *
 *      Filename:  ngx_vformatter.c
 *
 *      Description:  
 *
 *      Created:  11/06/14 17:12:12
 *
                     ...
                     register size_t *len)
{
     ...
     */
    if ((magnitude <= NGX_MAX_UINT32_VALUE && is_unsigned)
        || (num <= NGX_MAX_INT32_VALUE && num >= NGX_INT32_MIN && !is_unsigned))
...
static char *ngx_cvt(double arg, int ndigits, int *decpt, int *sign, 
                     int eflag, char *buf)
{
    register int r2;
    double fi, fj;
...
static char *ngx_ecvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (ngx_cvt(arg, ndigits, decpt, sign, 1, buf));
}

static char *ngx_fcvt(double arg, int ndigits, int *decpt, int *sign, char *buf)
{
    return (ngx_cvt(arg, ndigits, decpt, sign, 0, buf));
}

/*
 * ngx_gcvt  - Floating output conversion to
 * minimal length string
 */

static char *ngx_gcvt(double number, int ndigit, char *buf, boolean_e altform)
{
    int sign, decpt;
    register char *p1, *p2;
    register int i;
    ...
    p1 = ngx_ecvt(number, ndigit, &decpt, &sign, buf1);
    p2 = buf;
    if (sign)
        *p2++ = '-';
    ...
    char *buf, size_t *len)
{
    ...
    if (format == 'f')
        p = ngx_fcvt(num, precision, &decimal_point, is_negative, buf1);
    else /* either e or E format */
        p = ngx_ecvt(num, precision + 1, &decimal_point, is_negative, buf1);
...
static char *conv_os_thread_t_hex(ngx_thread_t *tid, char *buf_end, size_t *len)
{
    union {
        ngx_tid_t mtid;
        uint64_t u64;
...
static char *conv_os_thread_t(ngx_thread_t *tid, char *buf_end, size_t *len)
{
    union {
        ngx_tid_t mtid;
        uint64_t u64;
...
int ngx_vformatter(int (*flush_func)(ngx_vformatter_buff_t *),
    ngx_vformatter_buff_t *vbuff, const char *fmt, va_list ap)
{
    register char *sp;
    register char *bep;
    register int cc = 0;
    ...
    while (*fmt) {
        if (*fmt != '%') {
            ...
            /*
             * Modifier check.  In same cases, NGX_OFF_T_FMT can be
             * "lld" and NGX_INT64_T_FMT can be "ld" (that is, off_t is
             * "larger" than int64). Check that case 1st.
             * Note that if NGX_OFF_T_FMT is "d",
             * the first if condition is never true. If NGX_INT64_T_FMT
             * is "d' then the second if condition is never true.
             */
            if ((sizeof(NGX_OFF_T_FMT) > sizeof(NGX_INT64_T_FMT)) &&
                ((sizeof(NGX_OFF_T_FMT) == 4 &&
                 fmt[0] == NGX_OFF_T_FMT[0] &&
                 fmt[1] == NGX_OFF_T_FMT[1]) ||
                (sizeof(NGX_OFF_T_FMT) == 3 &&
                 fmt[0] == NGX_OFF_T_FMT[0]) ||
                (sizeof(NGX_OFF_T_FMT) > 4 &&
                 strncmp(fmt, NGX_OFF_T_FMT, 
                         sizeof(NGX_OFF_T_FMT) - 2) == 0))) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(NGX_OFF_T_FMT) - 2);
            }
            else if ((sizeof(NGX_INT64_T_FMT) == 4 &&
                 fmt[0] == NGX_INT64_T_FMT[0] &&
                 fmt[1] == NGX_INT64_T_FMT[1]) ||
                (sizeof(NGX_INT64_T_FMT) == 3 &&
                 fmt[0] == NGX_INT64_T_FMT[0]) ||
                (sizeof(NGX_INT64_T_FMT) > 4 &&
                 strncmp(fmt, NGX_INT64_T_FMT, 
                         sizeof(NGX_INT64_T_FMT) - 2) == 0)) {
                /* Need to account for trailing 'd' and null in sizeof() */
                var_type = IS_QUAD;
                fmt += (sizeof(NGX_INT64_T_FMT) - 2);
            }
            else if (*fmt == 'q') {
                var_type = IS_QUAD;
                fmt++;
            ...
            case 'i':
                if (var_type == IS_QUAD) {
                    i_quad = va_arg(ap, ngx_int64_t);
                    s = conv_10_quad(i_quad, 0, &is_negative,
                            &num_buf[NUM_BUF_SIZE], &s_len);
                }
                else {
                    if (var_type == IS_LONG)
                        i_num = va_arg(ap, ngx_int32_t);
                    else if (var_type == IS_SHORT)
            ...
            case 'G':
                 ...
                 * * We use &num_buf[ 1 ], so that we have room for the sign
                 */
                s = ngx_gcvt(va_arg(ap, double), (int) precision, &num_buf[1],
                            alternate_form);
                if (*s == '-')
                    prefix_char = *s++;
                else if (print_sign)
            ...
            case 'p':
                ...
                case 'p':
                    ...
                    if (sizeof(void *) <= sizeof(uint32_t)) {
                        ui_num = (uint32_t) va_arg(ap, void *);
                ...
                #if 0
                /* print an ngx_sockaddr_t as a.b.c.d:port */
                case 'I':
                {
                    struct sockaddr_in *sa;
                    //ngx_sockaddr_t *sa;
                    sa = va_arg(ap, struct sockaddr_in *);
                    if (sa != NULL) {
                        s = conv_ngx_sockaddr(sa, &num_buf[NUM_BUF_SIZE], &s_len);
                ...
                break;
                #endif
                /* print the error for an ngx_status_t */
                case 'm':
                {
                    ngx_int_t *mrv;

                    mrv = va_arg(ap, ngx_int_t *);
                    if (mrv != NULL) {
                        s = ngx_strerror(*mrv, num_buf, NUM_BUF_SIZE-1);
                        s_len = strlen(s);
                    }
                    else {
                        s = S_NULL;
