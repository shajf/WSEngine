...
ngx_file_t* ngx_open_file_read(ngx_pool_t *p,const char* fname){
    
    ngx_file_t *file;

    if(!fname) return NULL;

    file = (ngx_file_t *)ngx_palloc(p,sizeof(ngx_file_t));

    if(!file) return NULL;

    if (ngx_file_info(fname,&file->info)== NGX_FILE_ERROR) return NULL;
    
    file->fd = ngx_open_file(fname, NGX_FILE_RDONLY,NGX_FILE_OPEN, NGX_FILE_DEFAULT_ACCESS);

    if(file->fd==NGX_INVALID_FILE) return NULL;

    file->name.data = (u_char*)ngx_pstrndup(p,fname);
    file->name.len = ngx_strlen(file->name.data);
...
ngx_file_t* ngx_open_file_log(ngx_pool_t *p,const char* fname){
    ...
    file->fd = ngx_open_file(fname,NGX_FILE_APPEND,NGX_FILE_CREATE_OR_OPEN, NGX_FILE_DEFAULT_ACCESS);
...
ngx_int_t ngx_file_gets(char *str, int len, ngx_file_t *thefile)
{
    ...
    ssize_t fsize = ngx_file_size(&thefile->info);

    if(len <= 1||str==NULL) return NGX_ERROR;

    if(fsize==0) return NGX_EOF;

    while(str < final){
        
        if(thefile->offset == fsize)
        {
            rc = NGX_EOF;
            break;
        }

        nbytes = ngx_read_file(thefile,(u_char*)str,1, thefile->offset);
    ...
    *str = '\0';
    ...
    if(str > str_start) return NGX_OK;

    return rc;
}

ngx_int_t ngx_file_getc(char *ch,ngx_file_t *file){
	
    ssize_t fsize = ngx_file_size(&file->info);
	ssize_t nbytes;

	if(fsize == file->offset){
		*ch = '\0';
		return NGX_EOF;
	}

    nbytes = ngx_read_file(file,(u_char*)ch,1, file->offset);
