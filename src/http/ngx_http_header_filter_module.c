...
char * ngx_http_get_status_line(ngx_http_request_t *r,ngx_pool_t *p){
    
    ngx_str_t *status_line;
    size_t len = sizeof("HTTP/1.x ") - 1; 
    ...
    /* status line */
    if (status_line) {
        return ngx_psprintf(p,len+10,"%s %V","HTTP/1.1",status_line);
    } else {
        return ngx_psprintf(p,len+10,"%s %03ui","HTTP/1.1",status);
    }
}
