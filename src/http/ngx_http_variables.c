...
ngx_int_t  ngx_http_get_variable_v1(ngx_http_request_t *r, ngx_str_t *name,ngx_http_variable_value_t *vv)
{
    ngx_http_variable_t        *v;
    ngx_http_core_main_conf_t  *cmcf;
    ...
    if (v) {
        if (v->flags & NGX_HTTP_VAR_INDEXED) {
            ...
            tv = ngx_http_get_flushed_variable(r, v->index);

            if(tv==NULL) return NGX_ERROR;

            ngx_variable_value_copy(vv,tv);
...
u_char* ngx_http_pvdup(ngx_pool_t *p,ngx_http_variable_value_t *v){
    ...
    ngx_str_t vstr = {(size_t)v->len,v->data};
    
    return ngx_pstrdup(p,&vstr);
}
